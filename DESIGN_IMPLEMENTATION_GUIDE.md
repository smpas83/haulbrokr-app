...
- Request `status` is read-only to clients; designs should use explicit workflow actions instead of status editing.
...
- Dispatcher and fleet views should treat driver reassignment, live map routing, and traffic updates as limited beta items unless backend support is added.

## Accessibility expectations

- Preserve semantic forms, labels, keyboard navigation, focus states, skip-link behavior, zoom support, and reduced-motion behavior already present in the component system.
...
- Charts and analytics cards need text summaries for screen readers when final design packages are applied.
...
- QuickBooks/accounting screens should be labeled beta-disabled until real OAuth/sync behavior is implemented.
