...
import { useCustomerTimeline } from "@/hooks/useCustomerDashboardData";
import { cn } from "@/lib/utils";

function TimelineContent({ jobs }: { jobs: Job[] }) {
  const { data, isLoading, isError, refetch } = useCustomerTimeline(jobs);
          ...
          >
            <div className="px-3 py-2 border-b border-border bg-muted/30">
              ...
              <p className="text-xs text-muted-foreground capitalize truncate">
                {job.materialType} · {job.status.replace(/_/g, " ")}
              </p>
            </div>
