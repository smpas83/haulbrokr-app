...
import { useOperationsTimeline } from "@/hooks/useDispatcherData";
...
function TimelineContent({ jobs }: { jobs: Job[] }) {
  const { data, isLoading, isError, refetch } = useOperationsTimeline(jobs);
  ...
  if (!hasEvents) {
    return (
      <EmptyState
        ...
        description="Job status updates will appear chronologically, grouped by job."
  ...
  return (
    <div className="overflow-x-auto">
      <div className="flex gap-6 p-4 min-w-max">
        {groups.map(({ job, updates }) => (
          <div
            key={job.id}
            className="w-72 flex-shrink-0 border border-border bg-card/50 backdrop-blur-sm"
            role="group"
            aria-label={`Timeline for job ${job.id}`}
          >
            <div className="px-3 py-2 border-b border-border bg-muted/30">
              <p className="text-xs font-mono font-bold">JOB-{String(job.id).padStart(4, "0")}</p>
              <p className="text-xs text-muted-foreground capitalize truncate">{job.materialType} · {job.status.replace(/_/g, " ")}</p>
            </div>
...
export const OperationsTimeline = memo(function OperationsTimeline({
  jobs,
  open,
  onOpenChange,
