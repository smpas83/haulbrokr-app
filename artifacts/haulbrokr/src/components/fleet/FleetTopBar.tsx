...
import {
  Bell, ChevronDown, Cloud, Search, Truck, User,
} from "lucide-react";
...
interface FleetTopBarProps {
  companyName?: string;
  unreadCount?: number;
  hasMultipleFleets?: boolean;
}

export const FleetTopBar = memo(function FleetTopBar({
  companyName,
  unreadCount = 0,
  hasMultipleFleets = false,
}: FleetTopBarProps) {
  const { user } = useUser();
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 min-w-0 flex-shrink">
        ...
        <span className="font-black text-sm md:text-base truncate">{companyName ?? "Your Fleet"}</span>
      </div>

      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" size="sm" className="rounded-none border-2 hidden sm:flex gap-1">
            {hasMultipleFleets ? "Switch Fleet" : "Fleet Selector"}
        ...
        <DropdownMenuContent align="start" className="rounded-none border-2">
          {hasMultipleFleets ? (
            ...
            <DropdownMenuItem disabled className="text-muted-foreground">
              {/* PLACEHOLDER: ChatGPT visual package — multi-fleet selector */}
              Fleet selector unavailable
            </DropdownMenuItem>
          )}
        </DropdownMenuContent>
      </DropdownMenu>
