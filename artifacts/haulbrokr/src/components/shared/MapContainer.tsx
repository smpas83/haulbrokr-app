...
export interface MapMarker {
  ...
  lng: number;
  label: string;
  type: "truck" | "pickup" | "dropoff" | "cluster" | "facility";
  status?: string;
}
    ...
    <div className="relative h-full min-h-[320px] bg-muted/30 border-2 border-border overflow-hidden">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-xs text-muted-foreground mt-2 max-w-md">
          {/* PLACEHOLDER: ChatGPT visual package — interactive map with driver locations, pickup sites, dropoff facilities, active routes, ETA overlays */}
          Map infrastructure placeholder. Job and facility data loaded from existing APIs.
        </p>
      </div>

      <div className="absolute top-3 left-3 flex flex-wrap gap-2">
        <Badge variant="outline" className="rounded-none border-2 bg-card/90 backdrop-blur-sm">
          {trucks.length} trucks en route
        </Badge>
        <Badge variant="outline" className="rounded-none border-2 bg-card/90 backdrop-blur-sm">
          {activeJobs.length} active
        </Badge>
        <Badge variant="outline" className="rounded-none border-2 bg-card/90 backdrop-blur-sm">
          {completedJobs.length} delivered
        </Badge>
        {selectedJobId && (
...
export const MapContainer = memo(function MapContainer({
  ...
  description = "Driver locations, pickup sites, dropoff facilities, and active routes — awaiting map provider integration",
}: MapContainerProps) {
  return (
      ...
      <CardContent className="p-0">
        {isLoading ? (
          ...
          <EmptyState
            icon={MapPin}
            title="No map data"
            description="Active jobs and truck routes will appear on the operations map."
