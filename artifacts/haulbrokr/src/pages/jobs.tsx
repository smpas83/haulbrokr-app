      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {jobs.map((job) => (
            <div
              key={job.id}
              className="rounded-xl border border-border/60 bg-card hover:border-primary/30 hover:shadow-lg hover:shadow-black/10 transition-all flex flex-col"
            >
        ...
        <EmptyState
          ...
          action={{
            label: isCustomer ? "View Requests" : "Browse Load Board",
            href: "/requests",
          }}
        />
      )}
