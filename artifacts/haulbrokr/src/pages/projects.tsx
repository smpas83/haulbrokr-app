        ...
        <div className="space-y-4 mt-2">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, totalBudget: e.target.value }))
              }
              placeholder="50000"
            />
          </div>
    ...
    return (
      <div className="text-center py-20">
        ...
        <p className="text-muted-foreground mt-2">
          Switch to a customer account to manage projects.
        </p>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Group your hauling requests and track budgets per project
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="border border-dashed border-border/60 p-16 text-center">
          ...
          <p className="text-muted-foreground mb-6">
            Create a project to group your haul requests and track spend
          </p>
          <NewProjectDialog onCreated={() => {}} />
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {projects.map((p: any) => (
            <div
              key={p.id}
              className="bg-card border border-border/60 hover:border-primary/40 transition-colors group"
            >
              <div className="bg-secondary text-secondary-foreground p-4 flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <Badge
                      className={`rounded-xl border text-xs font-bold uppercase ${STATUS_COLORS[p.status]}`}
                    >
                      {p.status.replace("_", " ")}
                    </Badge>
                  </div>
                  ...
                  {p.siteAddress && (
                    <p className="text-xs text-secondary-foreground/60 mt-1 truncate">
                      {p.siteAddress}
                    </p>
                  )}
                </div>
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 transition-opacity ml-2 p-1 hover:text-destructive"
                >
              ...
              <div className="p-4 space-y-4">
                ...
                <Link href={`/projects/${p.id}`}>
                  <Button
                    variant="outline"
                    className="w-full rounded-xl border-2 font-bold text-xs uppercase tracking-wider mt-2 group-hover:border-primary/40"
                  >
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-card border border-border/60 p-4 text-center">
            ...
            <p className="text-3xl font-black">
              $
              {projects
                .reduce((s: number, p: any) => s + (p.totalBudget || 0), 0)
                .toLocaleString()}
            </p>
          </div>
          <div className="bg-card border border-border/60 p-4 text-center">
            ...
            <p className="text-3xl font-black">
              $
              {projects
                .reduce((s: number, p: any) => s + (p.spentAmount || 0), 0)
                .toLocaleString()}
            </p>
          </div>
        </div>
