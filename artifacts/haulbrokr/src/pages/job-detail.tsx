      ...
      <div className="flex items-center justify-between">
        ...
        {canUpload && (
          <Button
            size="sm"
            variant="outline"
            className="rounded-xl border-2 font-bold text-xs"
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {!editing && (
          <Button
            size="sm"
            variant="outline"
            className="rounded-xl border-2 font-bold text-xs flex-shrink-0"
            onClick={openEditor}
          >
...
function JobRatingPanel({
  ...
  const submit = useCreateJobRating({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: getGetJobRatingQueryKey(jobId),
        });
        toast({
      ...
      {isLoading ? (
        ...
        <div className="flex items-center gap-3">
          {[1, 2, 3, 4, 5].map((s) => (
            <Star
              key={s}
              className={`h-6 w-6 ${s <= mine.stars ? "fill-amber-400 text-amber-400" : "text-muted-foreground/30"}`}
            />
          ))}
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            {[1, 2, 3, 4, 5].map((s) => (
              ...
              >
                <Star
                  className={`h-8 w-8 ${s <= (hover || 0) ? "fill-amber-400 text-amber-400" : "text-muted-foreground/30"}`}
                />
              </button>
            ))}
          </div>
  ...
  return (
    <div className="border-t-2 border-border p-6 md:p-8 space-y-5">
      <div className="flex items-center justify-between">
        ...
        <Badge
          className={`rounded-xl border-2 font-bold uppercase text-xs px-3 py-1 ${paymentBadgeClass(status)}`}
        >
        ...
        {tickets.map((t) => (
          ...
          >
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <Badge variant="outline" className="rounded-xl text-xs uppercase">
                {t.status.replace("_", " ")}
              </Badge>
            </div>
            {t.weightTons != null && (
  ...
  return (
    <div className="border-t-2 border-border p-6 md:p-8 space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        ...
        job. This creates a load ticket the driver can act on.
      </p>
  ...
  return (
    <div className="border-t-2 border-border p-6 md:p-8 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Badge
          className={`rounded-xl border-2 font-bold uppercase text-xs px-3 py-1 ${
            approval === "approved"
        ...
        <div className="bg-secondary text-secondary-foreground p-6 md:p-8 flex flex-col md:flex-row justify-between items-start gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <Badge
                className={`rounded-xl border-2 font-bold uppercase text-xs px-3 py-1 ${getStatusColor(job.status)}`}
              >
