...
export default function FactoringPage() {
  ...
  const requestFunding = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Factoring request submitted",
        description: "Funds typically hit your account within 2 hours",
      });
    },
    ...
    return (
      <div className="text-center py-20">
        ...
        <h2 className="text-xl font-bold">
          Invoice Factoring is for providers
        </h2>
        <p className="text-muted-foreground mt-2">
          Switch to a provider account to access same-day advances.
        </p>
      </div>
    );
  }
  ...
  const eligibleJobs = (jobs as any[]).filter(
    (j: any) =>
      ...
      !factoredJobIds.has(j.id),
  );

  const totalAdvanced = requests
    .filter((r: any) => r.status !== "denied")
    .reduce((s: number, r: any) => s + r.netAmount, 0);
    ...
    <div className="max-w-5xl mx-auto space-y-6 page-enter pb-12">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Get paid same-day on completed jobs — no waiting for net-30 invoices
        </p>
      </div>
      ...
      <Alert className="rounded-xl border-2 border-primary/30 bg-primary/5">
        ...
        <AlertDescription className="text-sm space-y-1 mt-1">
          <p>
            1. You complete a job — submit a factoring request to get paid
            immediately
          </p>
          <p>
            2. HaulBrokr advances <strong>97%</strong> of the invoice (3%
            same-day fee)
          </p>
          <p>
            3. Funds hit your wallet within 2 hours. HaulBrokr collects from the
            customer at net-30
          </p>
        </AlertDescription>
      </Alert>
      ...
      {eligibleJobs.length > 0 && (
        <div>
          <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">
            Eligible for Advance ({eligibleJobs.length} jobs)
          </p>
          <div className="space-y-2">
            {eligibleJobs.map((job: any) => {
              ...
              return (
                <div
                  key={job.id}
                  className="flex items-center justify-between bg-card border border-border/60 p-4 hover:border-primary/40 transition-colors"
                >
                  <div>
                    <p className="font-bold">
                      JOB-{String(job.id).padStart(4, "0")} — {job.materialType}{" "}
                      Haul
                    </p>
                    <p className="text-sm text-muted-foreground mt-0.5">
                      Invoice: <strong>${invoiceAmt.toLocaleString()}</strong> →
                      You receive{" "}
        ...
        {isLoading ? (
          ...
          <div className="border border-dashed border-border/60 p-10 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Complete a job and request same-day funding above
            </p>
          </div>
        ) : (
          <div className="space-y-2">
            {requests.map((r: any) => {
              ...
              return (
                <div
                  key={r.id}
                  className="flex items-center justify-between bg-card border border-border/60 p-4"
                >
                  <div className="flex items-center gap-3">
                    <Icon className="h-5 w-5 text-muted-foreground" />
                    <div>
                      <p className="font-bold">
                        JOB-{String(r.jobId).padStart(4, "0")} — Invoice $
                        {r.invoiceAmount.toLocaleString()}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        Advanced ${r.netAmount.toFixed(2)} (fee: $
                  ...
                  <Badge
                    className={`rounded-xl border font-bold uppercase text-xs px-3 ${STATUS_COLORS[r.status]}`}
                  >
                    {r.status}
                  </Badge>
                </div>
