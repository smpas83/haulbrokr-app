...
const LIFECYCLE: { key: string; label: string; desc: string }[] = [
  {
    key: "pending",
    label: "Order Placed",
    desc: "Your request was submitted and is awaiting confirmation.",
  },
  {
    key: "confirmed",
    label: "Confirmed",
    desc: "A provider confirmed your order and is scheduling delivery.",
  },
  {
    key: "delivered",
    label: "Delivered",
    desc: "The bin was dropped off and is ready to use.",
  },
  {
    key: "picked_up",
    label: "Picked Up",
    desc: "The bin was hauled away. All done!",
  },
];
    ...
    <div className="max-w-3xl mx-auto space-y-6 page-enter pb-12">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">
                ...
                {order.updatedAt
                  ? ` on ${format(new Date(order.updatedAt), "MMM d, yyyy")}`
                  : ""}
                .
              </p>
            </div>
                ...
                <li key={step.key} className="flex gap-3">
                    ...
                    {done ? (
                      <CheckCircle
                        className={cn(
                          ...
                          current ? "text-primary" : "text-green-600",
                        )}
                      />
                    ) : (
                    ...
                    {!isLast && (
                      <div
                        className={cn(
                          ...
                          idx < currentIdx ? "bg-green-600/40" : "bg-border",
                        )}
                      />
                    )}
                  </div>
                  <div className={cn("pb-6", isLast && "pb-0")}>
                    ...
                    >
                      {step.label}
                      {current && (
                        <span className="ml-2 text-[10px] uppercase tracking-wider text-primary font-black">
                          Current
                        </span>
                      )}
                    </p>
                    ...
                    {stepDate && (
                      <p className="text-xs text-muted-foreground/70 mt-1">
                        {step.key === "delivered" && order.status === "pending"
                          ? "Scheduled: "
                          : ""}
      ...
      <div className="bg-card border border-border/60 p-6 space-y-4">
        ...
        <DetailRow
          icon={Package}
          label="Waste Type"
          value={WASTE_LABEL[order.wasteType] ?? order.wasteType}
        />
        <DetailRow
          icon={Truck}
          label="Preferred Provider"
          value={
            PROVIDER_LABEL[order.preferredProvider ?? "any"] ??
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    You can cancel this order while it hasn't been delivered
                    yet.
                  </p>
                </div>
              </div>
