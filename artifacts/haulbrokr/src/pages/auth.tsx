...
function AuthShell({ children, title, subtitle }: { children: ReactNode; title: string; subtitle: string }) {
  return (
    <div className="min-h-[100dvh] grid lg:grid-cols-2 bg-background">
      ...
      <aside className="hidden lg:flex flex-col justify-between p-12 border-r border-border/60 bg-card/30">
        <div>
          ...
          <p className="mt-4 text-muted-foreground text-lg max-w-md leading-relaxed">
            Mission control for contractors, fleet owners, and drivers — one platform from bid to payout.
          </p>
        </div>
      ...
      <div id="auth-form" className="flex flex-col items-center justify-center px-4 py-12">
        <div className="w-full max-w-[440px]">
          <div className="mb-8 lg:hidden text-center">
            <picture>
              <source type="image/webp" srcSet={logoWebp} />
              <img src={logo} alt="HaulBrokr" className="h-8 w-auto mx-auto" width="400" height="225" />
            </picture>
          </div>
...
export function SignInPage() {
  ...
  return (
    <AuthShell title="Welcome back" subtitle="Sign in to your mission control dashboard.">
      <SignIn
        routing="path"
...
export function SignUpPage() {
  return (
    <AuthShell title="Join the network" subtitle="Create your account — customer, vendor, or driver.">
      <SignUp
        routing="path"
