...
function ForemanAssignments({ projectId }: { projectId: number }) {
  ...
  const supervisors = (membersResp?.members ?? []).filter(
    (m) => m.role === "supervisor",
  );

  const invalidate = () =>
    qc.invalidateQueries({
      queryKey: getListProjectAssignmentsQueryKey(projectId),
    });
  ...
  const create = useCreateProjectAssignment({
    mutation: {
      onSuccess: () => {
        invalidate();
        setSelected("");
        toast({ title: "Foreman assigned to site" });
      },
      onError: (e: any) =>
        toast({
          title: "Failed to assign foreman",
  ...
  const remove = useRemoveProjectAssignment({
    mutation: {
      onSuccess: () => {
        invalidate();
        toast({ title: "Foreman removed from site" });
      },
      onError: (e: any) =>
        toast({
          title: "Failed to remove foreman",
          description: e.message,
          variant: "destructive",
        }),
    },
  ...
  const handleAssign = () => {
    ...
    create.mutate({
      id: projectId,
      data: { supervisorProfileId: Number(selected) },
    });
  };
  ...
  return (
    <div className="bg-muted/30 p-6 border border-border/60 space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        Assign foremen (supervisors) to oversee this job site. They can track
        loads and approve completed jobs on your behalf.
      </p>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {assigned.map((a) => (
            <div
              key={a.id}
              className="flex items-center justify-between bg-card p-3 border border-border/60"
            >
              <div className="flex items-center gap-3">
                <HardHat className="h-4 w-4 text-muted-foreground" />
                <span className="font-bold text-sm">
                  {a.supervisorName || `Foreman #${a.supervisorProfileId}`}
                </span>
              </div>
              <Button
                size="icon"
                ...
                onClick={() =>
                  remove.mutate({
                    id: projectId,
                    profileId: a.supervisorProfileId,
                  })
                }
              >
                <X className="h-4 w-4" />
          ...
          <SelectContent className="rounded-xl border-2">
            {available.map((s) => (
              <SelectItem key={s.id} value={String(s.id)}>
                {s.contactName || s.companyName || `Foreman #${s.id}`}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
    ...
    <div className="bg-muted/30 p-6 border border-border/60 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4 text-center">
        ...
        <div>
          ...
          <p
            className={`text-2xl font-black ${remaining < 0 ? "text-red-500" : "text-green-600"}`}
          >
      ...
      <div className="bg-card border border-border/60 overflow-hidden">
        <div className="bg-secondary text-secondary-foreground p-6 md:p-8 flex flex-col md:flex-row justify-between items-start gap-4">
          <div>
            {editing ? (
              <Input
                className="rounded-xl text-2xl font-bold bg-white/10 border-white/20 text-white placeholder:text-white/50 mb-2"
                value={editForm.name}
                onChange={(e) =>
            ...
            {project.siteAddress && (
              <p className="flex items-center gap-2 text-secondary-foreground/70 mt-1">
          ...
          <div className="flex items-center gap-3">
            {editing ? (
              ...
              >
                <SelectTrigger className="rounded-xl bg-white/10 border-white/20 text-white w-36">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              <Badge
                className={`rounded-xl border-2 font-bold uppercase text-xs px-3 py-1 ${STATUS_COLORS[project.status]}`}
              >
        ...
        <div className="p-6 md:p-8 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-muted/30 p-4 border border-border/60 text-center">
              ...
              <p className="text-2xl font-black">
                {project.totalBudget
                  ? `${((project.spentAmount / project.totalBudget) * 100).toFixed(0)}%`
                  : "—"}
              </p>
            </div>
          </div>
          ...
          {project.requests?.length > 0 && (
            <div>
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">
                Linked Haul Requests ({project.requests.length})
              </p>
              <div className="space-y-2">
                {project.requests.map((r: any) => (
                  <div
                    key={r.id}
                    className="flex items-center justify-between bg-muted/30 p-3 border border-border"
                  >
                    <div className="flex items-center gap-3">
