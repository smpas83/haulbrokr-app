...
async function resolveMetroPort() {
  const envPort = process.env.METRO_PORT || process.env.RCT_METRO_PORT;
  if (envPort) {
    const parsed = Number(envPort);
    if (!Number.isInteger(parsed) || parsed <= 0 || parsed > 65535) {
      throw new Error(`Invalid METRO_PORT: ${envPort}`);
    }
    return parsed;
  }
  return findFreePort();
...
async function startMetro(expoPublicDomain, expoPublicReplId, options = {}) {
  const port = options.port ?? metroPort;
  const spawnFn = options.spawn ?? spawn;
  const maxAttempts = options.maxAttempts ?? 60;
  const pollIntervalMs = options.pollIntervalMs ?? 1000;

  const isRunning = await checkMetroHealth(port);
  if (isRunning) {
  ...
  throw new Error(
    `Metro never became ready after ${maxAttempts} attempt(s) ` +
      `(polling every ${pollIntervalMs}ms on port ${port}). ` +
      "Metro failed to start or report healthy in time.",
  );
}
...
async function downloadBundlesAndManifests(timestamp, options = {}) {
  const downloadBundleFn = options.downloadBundle ?? downloadBundle;
  const downloadManifestFn = options.downloadManifest ?? downloadManifest;
  ...
  try {
    ...
    await downloadBundleFn("ios", timestamp);
    await downloadBundleFn("android", timestamp);

    const [iosManifest, androidManifest] = await Promise.all([
      downloadManifestFn("ios"),
      downloadManifestFn("android"),
    ]);
...
async function downloadAssets(assets, timestamp, options = {}) {
  const port = options.port ?? metroPort;
  ...
  const downloadPromises = assets.map(async (asset) => {
    const tempUrl = new URL(`http://localhost:${port}${asset.originalPath}`);
    const unstablePath = tempUrl.searchParams.get("unstable_path");
...
function updateManifests(manifests, timestamp, baseUrl, assetsByHash) {
  const updateForPlatform = (platform, manifest) => {
    if (!manifest.launchAsset || !manifest.extra) {
      throw new Error(`Malformed manifest for ${platform}`);
    }
...
async function main() {
  ...
  metroPort = await resolveMetroPort();

  prepareDirectories(timestamp);
  clearMetroCache();

  await startMetro(domain, expoPublicReplId, { port: metroPort });
