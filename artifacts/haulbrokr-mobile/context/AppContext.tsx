...
interface AppContextType {
  ...
  markAllRead: () => void;
  fleet: FleetTruck[];
  updateTruckStatus: (id: string, status: FleetTruck["status"]) => void;
  team: TeamMember[];
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const checkIn = (jobId: string) => {
    ...
    setJobs((prev) =>
      prev.map((j) => (j.id === jobId ? { ...j, checkInTime: time } : j)),
    );
  };
  ...
  const checkOut = (jobId: string) => {
    ...
    setJobs((prev) =>
      prev.map((j) => (j.id === jobId ? { ...j, checkOutTime: time } : j)),
    );
  };
  ...
  const cancelJob = (jobId: string, reason: string) =>
    setJobs((prev) =>
      prev.map((j) =>
        j.id === jobId
          ? { ...j, status: "cancelled", cancellationReason: reason }
          : j,
      ),
    );
  ...
  const rateJob = (jobId: string, rating: number) =>
    setJobs((prev) =>
      prev.map((j) => (j.id === jobId ? { ...j, myRating: rating } : j)),
    );

  const addLoadTicket = (jobId: string, ticket: Omit<LoadTicket, "id">) => {
    const id = Date.now().toString();
    const newTicket: LoadTicket = {
      ...ticket,
      id,
    ...
    setJobs((prev) =>
      prev.map((j) =>
        j.id === jobId
          ? { ...j, loadTickets: [...(j.loadTickets ?? []), newTicket] }
          : j,
      ),
    );
    return id;
    ...
    setJobs((prev) =>
      prev.map((j) =>
        j.id !== jobId
          ...
          : {
              ...j,
              loadTickets: (j.loadTickets ?? []).map((t) =>
                t.id === ticketId
                  ? {
                      ...t,
                      clockedInAt: time,
      ...
      prev.map((j) =>
        j.id !== jobId
          ...
          : {
              ...j,
              loadTickets: (j.loadTickets ?? []).map((t) =>
                t.id === ticketId ? { ...t, qrToken: token } : t,
              ),
            },
      ),
    );
  ...
  const verifyTicketByToken = (qrToken: string, verifierName: string) => {
    ...
    if (!job || !ticket) {
      return {
        ok: false,
        error: "Ticket not found. It may belong to another company.",
      };
    }
    if (ticket.verifiedAt) {
      return {
        ok: false,
        error: `Ticket #${ticket.loadNumber} was already verified at ${ticket.verifiedAt}.`,
      };
    }
    const time = new Date().toLocaleTimeString("en-US", {
      hour: "numeric",
          ...
          : {
              ...
              loadTickets: (j.loadTickets ?? []).map((t) =>
                t.id === ticketId
                  ? {
                      ...t,
                      status: "verified" as TicketStatus,
                      verifiedAt: time,
  ...
  const fileDispute = (jobId: string, reason: string) =>
    setJobs((prev) =>
      prev.map((j) => (j.id === jobId ? { ...j, disputeReason: reason } : j)),
    );
