...
export default function OnboardingScreen() {
  ...
  const ownerCompanyLabel = role === "provider" ? "Trucking company name *" : "Construction company name *";
  const num = (s: string) => (s.trim() ? Number(s) : undefined);

  const handleSubmit = async () => {
    if (!role) { Alert.alert("Select a role", "Choose how you'll use HaulBrokr."); return; }
    if (needsInvite) {
      if (!inviteCode.trim()) { Alert.alert("Invite code required", "Enter the code your manager shared with you."); return; }
    } else {
      if (!companyName.trim()) { Alert.alert("Company name required", "Please enter your company name."); return; }
    }
    ...
    try {
      await createProfile.mutateAsync({
        role,
        companyName: needsInvite ? "Pending team assignment" : companyName.trim(),
        ...
        ...(isProvider ? {
          ...
          equipmentTypes: equipmentTypes.length ? equipmentTypes.join(",") : undefined,
        } : {}),
        ...(isCustomer ? {
      ...
      const isAuthOrNetwork =
        /unauthor|401|403|network|fetch|failed to fetch|ecconnref|timeout/i.test(msg);
      if (needsInvite && !isAuthOrNetwork) {
        // Real backend rejected the invite code — surface it and stop.
        Alert.alert(
          "Couldn't join team",
          msg || "That invite code didn't match an organization. Double-check with your manager and try again."
        );
        return;
      }
      // Auth/network unavailable in demo preview — continue with local demo state.
      if (!isAuthOrNetwork) {
        Alert.alert("Saved locally", msg || "We saved your setup on this device. Some features will sync once you're signed in.");
      }
      router.replace("/" as any);
    }
  };
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.labelRow}>
            ...
            {role && (
              <Pressable
                onPress={() => { setRole(null); setCompanyName(""); setInviteCode(""); }}
                hitSlop={10}
              >
          ...
          {needsInvite ? (
            ...
            <>
              ...
              <Text style={styles.helperText}>You'll get an invite code to share with your team.</Text>
            </>
          ) : null}
            ...
            <>
              ...
              <View style={styles.row2}>
                <View style={styles.col}>
                  ...
                  <View style={styles.inputRow}>
                    <TextInput style={styles.input} placeholder="20" placeholderTextColor="#4a5568" value={capacityTons} onChangeText={setCapacityTons} keyboardType="numeric" />
                  </View>
                </View>
                <View style={styles.col}>
                  <Text style={styles.label}>Capacity (yards)</Text>
                  <View style={styles.inputRow}>
                    <TextInput style={styles.input} placeholder="16" placeholderTextColor="#4a5568" value={capacityYards} onChangeText={setCapacityYards} keyboardType="numeric" />
                  </View>
                </View>
              </View>
              ...
              <View style={styles.row2}>
                <View style={styles.col}>
                  ...
                  <View style={styles.inputRow}>
                    <TextInput style={styles.input} placeholder="125" placeholderTextColor="#4a5568" value={hourlyRate} onChangeText={setHourlyRate} keyboardType="numeric" />
                  </View>
                </View>
                <View style={styles.col}>
                  <Text style={styles.label}>Minimum hours</Text>
                  <View style={styles.inputRow}>
                    <TextInput style={styles.input} placeholder="4" placeholderTextColor="#4a5568" value={minimumHours} onChangeText={setMinimumHours} keyboardType="numeric" />
                  </View>
                </View>
              </View>
              ...
              <Text style={styles.helperText}>Select every truck and machine type you run.</Text>
            </>
          )}
          ...
          {isCustomer && (
            <>
              ...
              <Text style={styles.label}>Preferred payment terms (optional)</Text>
              <View style={styles.termsRow}>
                {["due_on_receipt", "net_15", "net_30"].map((t) => {
                  const active = paymentTerms === t;
                  const lbl = t === "due_on_receipt" ? "Due on receipt" : t === "net_15" ? "Net 15" : "Net 30";
                  return (
                    <Pressable key={t} onPress={() => setPaymentTerms(active ? "" : t)} style={[styles.termChip, active && styles.termChipActive]}>
                      <Text style={[styles.termChipText, active && styles.termChipTextActive]}>{lbl}</Text>
                    </Pressable>
                  );
                })}
              </View>
              <Text style={styles.helperText}>Net terms require credit approval, which you can apply for after signup.</Text>
            </>
          )}
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 13, fontFamily: "Inter_700Bold", color: "#FF5500", letterSpacing: 0.5, marginTop: 14, marginBottom: 10, textTransform: "uppercase" },
  ...
  equipChipActive: { borderColor: "#FF5500", backgroundColor: "#FF5500" },
  ...
  termChipActive: { borderColor: "#FF5500", backgroundColor: "#FF5500" },
  ...
  changeLink: { fontSize: 12, fontFamily: "Inter_600SemiBold", color: "#FF5500" },
  ...
  roleCardActive: { borderColor: "#FF5500", backgroundColor: "#FF5500" },
  ...
  btn: {
    backgroundColor: "#FF5500", borderRadius: 12, height: 52,
