...
export default function JobDetailScreen() {
  ...
  const quotePreview = useMarketplaceQuote();
  ...
  useEffect(() => {
    if (!showBidForm || !isRequestId || !liveRequest || bidRate) return;
    const estimatedHours = Number(liveRequest.estimatedHours ?? 0);
    if (!estimatedHours) return;
    const trucksNeeded = Number(liveRequest.trucksNeeded ?? 1);
    const budgetPerHour =
      liveRequest.budgetPerHour != null
        ? Number(liveRequest.budgetPerHour)
        : undefined;
    const quantityTons =
      liveRequest.quantityTons != null
        ? Number(liveRequest.quantityTons)
        : undefined;
    const truckType =
      typeof liveRequest.truckType === "string"
        ? liveRequest.truckType
        : undefined;
    const materialType = liveRequest.materialType ?? undefined;
    quotePreview.mutate(
      {
        distanceMiles: 0,
        estimatedHours,
        trucksNeeded,
        baseRatePerHour: budgetPerHour,
        truckType,
        materialType,
        quantityTons,
      },
      {
        onSuccess: (quote: any) => {
          if (quote?.vendorPayout && estimatedHours > 0) {
            setBidRate(String(Math.round(quote.vendorPayout / estimatedHours)));
          }
        },
      },
    );
  }, [showBidForm, isRequestId, liveRequest, bidRate]);
                          ...
                          Alert.alert(
                            ...
                            [
                              { text: "Cancel", style: "cancel" },
                              {
                                ...
                                style: "default",
                                onPress: () => {
                                  const liveBidId = Number(bid.id);
                                  if (
                                    isRequestId &&
                                    Number.isFinite(liveBidId)
                                  ) {
                                    updateBid.mutate(
                                      { id: liveBidId, status: "accepted" },
                                      {
                                        onSuccess: () => {
