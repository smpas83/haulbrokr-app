...
export default function ForemanScreen() {
  ...
  return (
    <>
      ...
      >
        <Text style={[styles.title, { color: colors.foreground }]}>Site Jobs</Text>
        <Text style={[styles.subtitle, { color: colors.mutedForeground }]}>
          Track drivers on site, verify tickets, and approve or flag completed work.
        </Text>
        ...
        >
          ...
          <Text style={[styles.scanBtnText, { color: colors.primaryForeground }]}>Scan Driver Ticket</Text>
        </Pressable>
        ...
        {jobsQuery.isLoading ? (
          ...
          <View style={[styles.emptyCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <Text style={[styles.emptyTitle, { color: colors.foreground }]}>No active site jobs</Text>
            <Text style={[styles.emptyBody, { color: colors.mutedForeground }]}>
              Jobs hauling to your sites will appear here so you can supervise drivers and sign off on the work.
            </Text>
          </View>
        ) : (
          siteJobs.map((job) => (
...
function ForemanJobCard({
  ...
  const handleApprove = () => {
    Alert.alert("Approve completion", "Confirm this job was completed satisfactorily?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: () =>
          approve.mutate(job.id, {
            ...
            onError: (e: any) => Alert.alert("Couldn't approve", e?.message ?? "Try again."),
          }),
      },
    ]);
  };
  ...
  const handleFlag = () => {
    if (!reason.trim()) {
      Alert.alert("Reason required", "Describe the issue before flagging this job.");
      return;
    }
    flag.mutate(
      { jobId: job.id, reason: reason.trim() },
      {
        ...
        onError: (e: any) => Alert.alert("Couldn't flag", e?.message ?? "Try again."),
      },
    );
  };

  const approvalColor = approval === "approved" ? ACCENT.green : approval === "flagged" ? ACCENT.red : colors.mutedForeground;
  const approvalLabel = approval === "approved" ? "Approved" : approval === "flagged" ? "Flagged" : "Pending";
    ...
    >
      <View style={styles.cardHeader}>
        ...
        <View style={[styles.approvalBadge, { backgroundColor: approvalColor + "20" }]}>
          <Text style={[styles.approvalBadgeText, { color: approvalColor }]}>{approvalLabel}</Text>
        </View>
      </View>
      ...
      {expanded && (
        <View style={[styles.body, { borderTopColor: colors.border }]}>
          {/* Current status */}
          <Text style={[styles.sectionLabel, { color: colors.mutedForeground }]}>DRIVER STATUS</Text>
          {latest ? (
            <View style={[styles.statusRow, { backgroundColor: colors.background, borderColor: colors.border }]}>
              <View style={[styles.statusDot, { backgroundColor: ACCENT.green }]} />
              <Text style={[styles.statusText, { color: colors.foreground }]}>
                {STATUS_LABEL[latest.status] ?? latest.status}
              </Text>
              <Text style={[styles.statusBy, { color: colors.mutedForeground }]} numberOfLines={1}>
                {latest.actorName ?? "Driver"}
              </Text>
            </View>
          ) : (
            <Text style={[styles.hint, { color: colors.mutedForeground }]}>No status reported yet.</Text>
          )}
          ...
          {tickets.length === 0 ? (
            <Text style={[styles.hint, { color: colors.mutedForeground }]}>No driver check-ins yet.</Text>
          ) : (
            tickets.map((t) => {
              const checkedOut = !!t.clockOutAt;
              const checkedIn = !!t.clockInAt;
              const state = checkedOut ? "Departed" : checkedIn ? "On site" : "Pending";
              const dot = checkedOut ? colors.mutedForeground : checkedIn ? ACCENT.green : colors.primary;
              return (
                <View key={t.id} style={[styles.ticketRow, { borderColor: colors.border, backgroundColor: colors.background }]}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={[styles.ticketMeta, { color: colors.mutedForeground }]}>
                      {state}{t.weightTons ? ` · ${t.weightTons} tons` : ""}
                    </Text>
                  </View>
                  {t.verifiedAt ? <Feather name="check-circle" size={18} color={ACCENT.green} /> : null}
                </View>
              );
            })
          )}
          ...
          {approval === "flagged" && job.flagReason ? (
            <View style={[styles.flagBanner, { backgroundColor: ACCENT.red + "12", borderColor: ACCENT.red + "40" }]}>
              <Feather name="alert-triangle" size={14} color={ACCENT.red} />
              <Text style={[styles.flagBannerText, { color: ACCENT.red }]}>{job.flagReason}</Text>
            </View>
          ) : null}

          {/* Approve / flag actions */}
          <Text style={[styles.sectionLabel, { color: colors.mutedForeground, marginTop: 16 }]}>SIGN OFF</Text>
          {flagOpen ? (
            <View>
              <TextInput
                value={reason}
              ...
              <View style={styles.btnRow}>
                <Pressable
                  onPress={() => { setFlagOpen(false); setReason(""); }}
                ...
                <Pressable
                  ...
                  style={[styles.dangerBtn, { backgroundColor: ACCENT.red, flex: 1, opacity: flag.isPending ? 0.6 : 1 }]}
                >
                  <Text style={styles.dangerBtnText}>{flag.isPending ? "Flagging…" : "Submit Flag"}</Text>
                </Pressable>
              </View>
            </View>
          ) : (
            <View style={styles.btnRow}>
              <Pressable
                ...
                style={[styles.approveBtn, { backgroundColor: ACCENT.green, flex: 1, opacity: approve.isPending ? 0.6 : 1 }]}
              >
                <Feather name="check" size={15} color="#fff" />
                <Text style={styles.approveBtnText}>{approve.isPending ? "Approving…" : "Approve"}</Text>
              </Pressable>
              <Pressable
                onPress={() => setFlagOpen(true)}
                style={[styles.outlineBtn, { borderColor: ACCENT.red + "60", flex: 1 }]}
              >
                <Feather name="flag" size={15} color={ACCENT.red} />
                <Text style={[styles.outlineBtnText, { color: ACCENT.red }]}>Flag Issue</Text>
              </Pressable>
            </View>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 13, fontFamily: "Inter_400Regular", marginBottom: 16, lineHeight: 19 },
  scanBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 13, borderRadius: 10, marginBottom: 18 },
  ...
  statusRow: { flexDirection: "row", alignItems: "center", gap: 10, borderWidth: 1, borderRadius: 10, padding: 12 },
  statusDot: { width: 10, height: 10, borderRadius: 5 },
  statusText: { fontSize: 14, fontFamily: "Inter_600SemiBold" },
  statusBy: { fontSize: 12, fontFamily: "Inter_400Regular", flex: 1, textAlign: "right" },
  ...
  flagBanner: { flexDirection: "row", alignItems: "center", gap: 8, padding: 12, borderRadius: 10, borderWidth: 1, marginTop: 12 },
  flagBannerText: { fontSize: 13, fontFamily: "Inter_500Medium", flex: 1 },
  reasonInput: { borderWidth: 1, borderRadius: 10, padding: 12, fontSize: 14, fontFamily: "Inter_400Regular", minHeight: 70, textAlignVertical: "top", marginBottom: 10 },
  btnRow: { flexDirection: "row", gap: 10 },
  approveBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 13, borderRadius: 10 },
  approveBtnText: { fontSize: 14, fontFamily: "Inter_700Bold", color: "#fff" },
  dangerBtn: { alignItems: "center", justifyContent: "center", paddingVertical: 13, borderRadius: 10 },
