...
export default function AccountScreen() {
  ...
  const failedPaymentCount = Array.isArray(activityQuery.data)
    ? activityQuery.data.filter(
        (a: { type?: string }) => a.type === "payment_failed",
      ).length
    : 0;

  const compliancePending = Array.isArray(adminCompliance.data)
    ? adminCompliance.data.filter(
  ...
  const creditPending = Array.isArray(adminCredit.data)
    ...
    : 0;
  const stuckPayoutsCount = Array.isArray(stuckPayouts.data)
    ? stuckPayouts.data.length
    : 0;
  ...
  const handleSignOut = useCallback(async () => {
    ...
    try {
      ...
      await performAccountSignOut({
        signOut,
        replace: (path) => router.replace(path as any),
        onError: (message) => Alert.alert("Sign out failed", message),
      });
    } finally {
      setSigningOut(false);
    }
  ...
  const handleFMCSAVerify = async () => {
    ...
    if (!dot) {
      Alert.alert(
        "Enter DOT Number",
        "Please enter your USDOT number to verify.",
      );
      return;
    }
    setVerifying(true);
    try {
      ...
      Alert.alert(
        "Submitted ✅",
        `DOT #${dot} submitted for verification. Status will update shortly.`,
      );
    } catch (err: any) {
  ...
  const handleRunVerify = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Verification failed",
        err?.message ?? "Could not run verification.",
      );
    } finally {
      setVerifying(false);
    }
  ...
  const handleSubmitCredit = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        err?.message ?? "Could not submit credit application.",
      );
    } finally {
  ...
  const handleQBConnect = async () => {
    if (!qbCompany.trim()) {
      Alert.alert(
        ...
        "Enter your QuickBooks company name.",
      );
      return;
    }
    try {
  ...
  const handleQBSync = async () => {
    try {
      ...
      Alert.alert(
        "Sync complete",
        `${result.invoicesSynced} invoice(s) synced. Total: ${result.totalSynced}.`,
      );
    } catch (err: any) {
  ...
  const handleQBDisconnect = () => {
    Alert.alert(
      ...
      "This will remove the QuickBooks connection.",
      [
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      >
        ...
        <Animated.View entering={FadeInDown.delay(0).springify()}>
          <View
            style={[styles.profileCard, { backgroundColor: colors.secondary }]}
          >
            <View style={styles.profileTop}>
              ...
              <View style={styles.profileInfo}>
                <Text
                  style={[
                    styles.profileName,
                    { color: "#f0f6ff", fontFamily: "Inter_700Bold" },
                  ]}
                >
                  {profile.name}
                </Text>
                <Text
                  style={[
                    ...
                    { color: "#8ba0b8", fontFamily: "Inter_400Regular" },
                  ]}
                >
                  {profile.company}
                </Text>
              ...
              <Pressable
                ...
                testID="account-header-sign-out"
            ...
            <View style={styles.profileMeta}>
              ...
              >
                <Feather
                  name={isProvider ? "truck" : "briefcase"}
                  size={12}
        ...
        {failedPaymentCount > 0 && (
          <Animated.View entering={FadeInDown.delay(20).springify()}>
            ...
            >
              ...
              >
                {failedPaymentCount} payment issue
                {failedPaymentCount !== 1 ? "s" : ""} need attention — tap to
                review
              </Text>
              <Feather
                name="chevron-right"
                ...
                <View>
                  <Text
                    style={[
                      styles.onlineTitle,
                      {
                        color: isOnline ? "#16a34a" : colors.foreground,
                        fontFamily: "Inter_700Bold",
                      },
                    ]}
                  >
        ...
        {/* Wallet Card */}
        <Animated.View
          entering={FadeInDown.delay(isProvider ? 80 : 40).springify()}
        >
          <Pressable
          ...
          >
            <View style={styles.walletLeft}>
              <View
                style={[styles.walletIcon, { backgroundColor: "#ffffff20" }]}
              >
            ...
            >
              <View style={styles.fmcsaRow}>
                ...
                >
                  <Feather
                    name="shield"
                    size={22}
                    color={dotVerified ? ACCENT.green : colors.mutedForeground}
                  />
                </View>
                <View style={{ flex: 1 }}>
                  <Text
                  ...
                  >
                    {dotVerified
                      ? "Verified with FMCSA — Active carrier"
                      : "Enter your DOT number to verify"}
                  </Text>
                </View>
                {dotVerified && (
                  <View
              ...
              <View style={{ gap: 6, paddingVertical: 4 }}>
                {[
                  {
                    key: "fmcsaAuthority",
                    label: "Operating authority active",
                  },
                  {
                    key: "insuranceActive",
                  ...
                  {
                    key: "dotOperatingStatus",
                    label: "DOT operating status OK",
                  },
                  {
                    key: "notSuspended",
                    label: "Not suspended / out-of-service",
                  },
                ].map((chk) => {
                  ...
                  return (
                    ...
                    >
                      <Feather
                        ...
                        size={14}
                        color={ok ? ACCENT.green : colors.mutedForeground}
                      />
                      <Text
                        style={[
                          {
                ...
                {liveCompliance?.safetyRating ? (
                  ...
                  >
                    Safety rating: {liveCompliance.safetyRating}
                  </Text>
                ) : null}
              </View>
              <Pressable
                ...
                style={[
                  ...
                  { backgroundColor: ACCENT.green + (verifying ? "80" : "") },
                ]}
              >
                <Text
                  style={{
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  >
                    {dotVerified
                      ? "CDL on file — Class A verified"
                      : "Submit CDL details in Account → DOT / CDL tab"}
                  </Text>
                </View>
                {dotVerified ? (
                  <View
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <Feather
                        name={enabled ? "check-circle" : "dollar-sign"}
                        size={18}
                        color={accent}
                      />
                    </View>
                  ...
                  {connected &&
                    ...
                    (() => {
                      ...
                      const deadline =
                        ps?.requirements?.currentDeadline ?? null;
                      return (
                              ...
                              {deadline && (
                                <Text
                                  style={[
                                    ...
                                    { color: ACCENT.red ?? colors.primary },
                                  ]}
                                >
                                  Complete by{" "}
                                  {new Date(
                            ...
                            pendingVerification.length === 0 && (
                              <View style={{ gap: 6 }}>
                                {[
                                  ...
                                  >
                                    <Feather
                                      name={chk.ok ? "check-circle" : "circle"}
                                      size={14}
                                      color={
                                        chk.ok
                                          ? ACCENT.green
                  ...
                  {!enabled && (
                    ...
                    >
                      ...
                      >
                        Payments from completed jobs can't be released until
                        payouts are enabled.
                      </Text>
                    </View>
                  )}
                  ...
                  <Pressable
                    ...
                    style={[
                      ...
                      },
                      {
                        ...
                        borderColor: colors.border,
                        opacity: connectStripe.isPending ? 0.7 : 1,
                      },
                    ]}
                  >
                    <Feather
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        {qbStatus.data.lastSyncedAt
                          ? ` · Last: ${new Date(qbStatus.data.lastSyncedAt).toLocaleDateString()}`
                          : ""}
                      </Text>
                    </View>
                    <View
            ...
            >
              <View
                style={[
                  styles.progressFill,
                  {
                    backgroundColor: pct >= 80 ? ACCENT.green : colors.primary,
                    width: `${pct}%` as any,
                  },
                ]}
              />
            ...
            <Pressable
              ...
              onPress={() => {
                ...
                if (!item.done)
                  Alert.alert(
                    item.label,
                    `Tap to upload or complete your ${item.label}.`,
                  );
              }}
              style={[
            ...
            >
              ...
              >
                <Feather
                  ...
                  size={16}
                  color={item.done ? "#16a34a" : colors.primary}
                />
              </View>
              <Text
                style={[
              ...
              {item.done ? (
                ...
                <View
                  style={[
                    styles.compPendingBadge,
                    { backgroundColor: colors.primary + "20" },
                  ]}
                >
                  <Text
                    style={[
                      styles.compPendingText,
                      {
                        color: colors.primary,
                        fontFamily: "Inter_600SemiBold",
                      },
          ...
          <Animated.View entering={FadeInDown.delay(140).springify()}>
            ...
            >
              {creditQuery.data?.status ? (
                ...
                >
                  Apply for Net invoicing so you can hire trucks now and pay
                  after the job.
                </Text>
              )}
              ...
              >
                <Feather
                  name={wantsInvoicing ? "check-square" : "square"}
                  size={20}
                  color={
                    wantsInvoicing ? colors.primary : colors.mutedForeground
                  }
                />
                <Text
                  style={[
              ...
              <Text
                style={[styles.creditLabel, { color: colors.mutedForeground }]}
              >
              ...
              >
                <TextInput
                  ...
                  onChangeText={setEstimatedMonthlySpend}
                  keyboardType="numeric"
                />
              </View>
              ...
              >
                <TextInput
                  ...
                  onChangeText={setTradeReferences}
                />
              </View>
              ...
              <Pressable
                ...
                style={[
                  ...
                  {
                    backgroundColor: colors.primary,
                    opacity: submittingCredit ? 0.6 : 1,
                  },
                ]}
              >
                <Text
          ...
          <View style={styles.statsRow}>
            {isProvider ? (
              <>
                <StatItem
                  label="Total Hauls"
                  value={profile.totalHauls.toString()}
                  colors={colors}
                />
        ...
        <Animated.View entering={FadeInDown.delay(260).springify()}>
          ...
          >
            ...
            />
            <View
              style={[styles.prefDivider, { backgroundColor: colors.border }]}
            />
            <NotifToggle
              label="SMS Alerts"
              icon="message-square"
            ...
            {unreadCount > 0 && (
              <>
                ...
                >
                  ...
                  >
                    View {unreadCount} unread notification
                    {unreadCount !== 1 ? "s" : ""}
                  </Text>
                  <Feather
                    name="chevron-right"
          ...
          {[
            ...
            ...(isOwner
              ? [
                  {
                    id: "team",
                    label: isProvider ? "Drivers & Team" : "Supervisors & Team",
                    icon: "users",
                    type: "nav" as const,
                  },
                ]
            ...
            <Pressable
              ...
              onPress={() => {
                Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                router.push(NAV_ROUTES[item.id] as any);
              }}
              style={[
                styles.settingItem,
                { backgroundColor: colors.card, borderColor: colors.border },
        ...
        <Animated.View entering={FadeInDown.delay(400).springify()}>
          <Pressable
            ...
            accessibilityRole="button"
            accessibilityLabel="Sign out"
            testID="account-footer-sign-out"
            style={[
              styles.signOutBtn,
              { backgroundColor: colors.card, borderColor: colors.border },
            ]}
