...
export default function HomeScreen() {
  ...
  const {
    ...
    isFetching: fetchingPayout,
    refetch: refetchPayout,
  } = usePayoutStatus();
  const payoutsNeedSetup =
    isProvider && payoutStatusData != null && !payoutStatusData.payoutsEnabled;
  ...
  const stats = {
    ...
    money: isProvider
      ? (liveStats?.totalRevenue ?? 0)
      : (liveStats?.totalSpent ?? 0),
  };
  ...
  const handleOnlineToggle = (val: boolean) => {
    ...
    if (val)
      Alert.alert(
        "You are Online",
        "You'll receive new job notifications in your area.",
      );
  };
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      >
          ...
          <View style={styles.headerRight}>
            ...
            >
              <Feather
                ...
                size={20}
                color={
                  unreadCount > 0 ? colors.primary : colors.mutedForeground
                }
              />
              {unreadCount > 0 && (
            ...
            >
              <View style={styles.onlineLeft}>
                <View
                  style={[
                    styles.onlinePulse,
                    { backgroundColor: isOnline ? "#16a34a" : "#6b728040" },
                  ]}
                />
                <View>
                  <Text
          ...
          <Animated.View entering={FadeInDown.delay(40).springify()}>
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                >
                  Connect a payout account with Stripe so you can receive money
                  from completed jobs.
                </Text>
              </View>
        ...
        >
          ...
          <StatCard
            icon="trending-up"
            value={isProvider ? stats.pendingBids : stats.totalBids}
            label={isProvider ? "My Bids" : "Bids Received"}
          />
          <StatCard
            icon="dollar-sign"
            value={fmtMoney(stats.money)}
            label={isProvider ? "Earnings" : "Jobs Paid"}
          />
        </ScrollView>
          ...
          {isProvider ? (
            ...
            <>
              ...
              <QuickAction
                icon="navigation"
                label="Dump Sites"
                onPress={() => router.push("/dump-sites")}
                colors={colors}
              />
              <QuickAction
                icon="credit-card"
        ...
        >
          {(showAllActivity ? activity : activity.slice(0, 3)).map(
            (item, idx, arr) => {
              const tappable = !!item.binOrderId;
              const openBinOrder = () => {
                Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                router.push(
                  `/bins?order=${encodeURIComponent(item.binOrderId!)}`,
                );
              };
              return (
                <View key={item.id}>
                  <Pressable
                    ...
                    style={({ pressed }) => [
                      styles.activityItem,
                      { opacity: tappable && pressed ? 0.6 : 1 },
                    ]}
                  >
                    <View
                      style={[
                    ...
                    >
                      <Feather
                        ...
                        size={14}
                        color={
                          ACTIVITY_ICON_COLORS[item.type] ?? colors.primary
                        }
                      />
                    </View>
  ...
  return (
    <Pressable
      ...
      style={[
        styles.action,
        {
          backgroundColor: highlight ? colors.primary + "10" : colors.card,
