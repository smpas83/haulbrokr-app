...
import { clerkErrorMessage, logClerkAuthError } from "@/lib/clerkAuthLogging";
import {
...
export default function SignInScreen() {
  ...
  useEffect(() => {
    if (__DEV__) {
      console.log(
        "[ClerkAuth] OAuth redirect URIs for this build:",
        clerkOAuthRedirectUriCandidates(),
      );
    }
  }, []);
  ...
  const handleSSO = async (strategy: "oauth_google" | "oauth_apple") => {
    ...
    try {
      ...
      logClerkAuthError(`${which}-sso`, err, {
        which,
        redirectUrl: clerkOAuthRedirectUri(),
      });
  ...
  const finalizeSignUp = async (): Promise<boolean> => {
    ...
    logClerkAuthError("signUp.finalize", error, {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      if (passwordError) {
        logClerkAuthError("signUp.password", passwordError, {
          email: nextEmail,
          username: nextUsername,
        });
    ...
    try {
      if (verifyReason === "trust") {
        ...
        if (signIn.status === "complete") {
          ...
          logClerkAuthError(
            "signIn.verifyMfaIncomplete",
            new Error("MFA verify succeeded but sign-in not complete"),
            {
              status: signIn.status,
            },
          );
          setError(
            "Additional verification is still required. Check your email or try signing in again.",
          );
        }
        return;
      }
      ...
      if (verifyError) {
        logClerkAuthError("signUp.verifyEmailCode", verifyError, {
          status: signUp.status,
        });
        setError(clerkErrorMessage(verifyError));
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
      ...
      if (signUp.status === "complete" || sessionId) {
        if (sessionId) {
          ...
          if (!activated && signUp.status !== "complete") {
            setError(
              "Verification succeeded but session activation failed. Please try again.",
            );
            return;
          }
        }
        ...
        logClerkAuthError(
          "signUp.verifyIncomplete",
          new Error("Email verify succeeded without session"),
          {
            status: signUp.status,
          },
        );
        setError(
          "Email verified but sign-up could not finish. Tap Resend or go back and try again.",
        );
      }
    } finally {
      setLoading(false);
