    ...
    return (
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        <View style={[styles.header, { backgroundColor: colors.background, borderBottomColor: colors.border, paddingTop: topPad + 12 }]}>
          ...
          <Text style={[styles.headerTitle, { color: colors.foreground, fontFamily: "Inter_700Bold" }]}>Invoice</Text>
          <View style={styles.shareBtn} />
        </View>
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
          <Feather name="file-text" size={40} color={colors.mutedForeground} style={{ marginBottom: 12 }} />
          <Text style={{ color: colors.foreground, fontFamily: "Inter_600SemiBold", fontSize: 17, marginBottom: 6 }}>Invoice Not Found</Text>
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.background, borderBottomColor: colors.border, paddingTop: topPad + 12 }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.foreground, fontFamily: "Inter_700Bold" }]}>Invoice</Text>
        <Pressable
          onPress={() => Alert.alert("Share Invoice", "Invoice PDF will be sent to your email and can be shared via the share sheet.")}
          style={[styles.shareBtn, { backgroundColor: colors.primary }]}
        >
          <Feather name="share" size={16} color={colors.primaryForeground} />
          <Text style={[styles.shareBtnText, { color: colors.primaryForeground, fontFamily: "Inter_600SemiBold" }]}>Share</Text>
        </Pressable>
      </View>
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: 40 + insets.bottom }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.invoice, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <View style={[styles.letterhead, { backgroundColor: colors.primary }]}>
            <View>
              <Text style={[styles.brandName, { color: colors.primaryForeground, fontFamily: "Inter_700Bold" }]}>HaulBrokr</Text>
              <Text style={[styles.brandTagline, { color: colors.primaryForeground + "cc", fontFamily: "Inter_400Regular" }]}>
                Professional Hauling Network
              </Text>
            </View>
            <View style={[styles.paidStamp, { borderColor: colors.primaryForeground + "60" }]}>
              <Text style={[styles.paidText, { color: colors.primaryForeground, fontFamily: "Inter_700Bold" }]}>
          ...
          {/* Invoice meta */}
          <View style={[styles.invoiceMeta, { borderBottomColor: colors.border }]}>
            <View style={{ gap: 4 }}>
              <Text style={[styles.metaLabel, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>INVOICE NUMBER</Text>
              <Text style={[styles.metaValue, { color: colors.foreground, fontFamily: "Inter_700Bold" }]}>{invoiceNum}</Text>
            </View>
            <View style={{ gap: 4, alignItems: "flex-end" }}>
              <Text style={[styles.metaLabel, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>ISSUE DATE</Text>
              <Text style={[styles.metaValue, { color: colors.foreground, fontFamily: "Inter_700Bold" }]}>{issueDate}</Text>
            </View>
          </View>

          {/* Bill to / Bill from */}
          <View style={[styles.partiesRow, { borderBottomColor: colors.border }]}>
            <View style={{ flex: 1 }}>
              <Text style={[styles.partyLabel, { color: colors.mutedForeground, fontFamily: "Inter_500Medium" }]}>CUSTOMER</Text>
              <Text style={[styles.partyName, { color: colors.foreground, fontFamily: "Inter_700Bold" }]}>{job.postedBy}</Text>
              <Text style={[styles.partySub, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>{job.pickupAddress.split(",").slice(-2).join(",").trim()}</Text>
            </View>
            <View style={[styles.partiesDivider, { backgroundColor: colors.border }]} />
            <View style={{ flex: 1 }}>
              <Text style={[styles.partyLabel, { color: colors.mutedForeground, fontFamily: "Inter_500Medium" }]}>PROVIDER</Text>
              {job.providerCompany ? (
                <>
                  <Text style={[styles.partyName, { color: colors.foreground, fontFamily: "Inter_700Bold" }]}>{job.providerCompany}</Text>
                  <Text style={[styles.partySub, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>Dallas, TX</Text>
                </>
              ) : (
                <>
                  <Text style={[styles.partyName, { color: colors.mutedForeground, fontFamily: "Inter_600SemiBold", fontStyle: "italic" }]}>Pending Assignment</Text>
                  <Text style={[styles.partySub, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>Awaiting bid acceptance</Text>
                </>
              )}
            </View>
          </View>
          ...
          <View style={[styles.section, { borderBottomColor: colors.border }]}>
            <Text style={[styles.sectionTitle, { color: colors.mutedForeground, fontFamily: "Inter_600SemiBold" }]}>JOB DETAILS</Text>
            <InvoiceRow label="Project" value={job.projectName} colors={colors} />
            <InvoiceRow label="Material" value={job.material} colors={colors} />
            {job.quantity > 0 && <InvoiceRow label="Quantity" value={`${job.quantity.toLocaleString()} ${job.quantityUnit}`} colors={colors} />}
            <InvoiceRow label="Trucks" value={`${job.trucksNeeded} truck${job.trucksNeeded !== 1 ? "s" : ""}`} colors={colors} />
            <InvoiceRow label="Date" value={job.scheduledDate} colors={colors} />
            {job.checkInTime && <InvoiceRow label="Check-In" value={job.checkInTime} colors={colors} />}
            {job.checkOutTime && <InvoiceRow label="Check-Out" value={job.checkOutTime} colors={colors} />}
            {totalTicketWeight > 0 && (
              <InvoiceRow label="Total Weight" value={`${totalTicketWeight.toFixed(1)} tons verified`} colors={colors} highlight />
            )}
          </View>
          ...
          {(job.loadTickets?.length ?? 0) > 0 && (
            <View style={[styles.section, { borderBottomColor: colors.border }]}>
              <Text style={[styles.sectionTitle, { color: colors.mutedForeground, fontFamily: "Inter_600SemiBold" }]}>LOAD TICKETS</Text>
              {job.loadTickets!.map((t) => (
                <View key={t.id} style={styles.ticketRow}>
                  <Text style={[styles.ticketNum, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>Load #{t.loadNumber}</Text>
                  <Text style={[styles.ticketTime, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>{t.timestamp}</Text>
                  <Text style={[styles.ticketWeight, { color: colors.foreground, fontFamily: "Inter_600SemiBold" }]}>{t.weight ?? "—"}</Text>
                  {t.hasPhoto && <Feather name="image" size={12} color={ACCENT.blue} />}
                </View>
              ))}
            </View>
          )}
          ...
          {/* Footer */}
          <View style={[styles.invoiceFooter, { borderTopColor: colors.border, backgroundColor: colors.background }]}>
...
function InvoiceRow({ label, value, colors, highlight }: { label: string; value: string; colors: any; highlight?: boolean }) {
  return (
    <View style={styles.infoRow}>
      <Text style={[styles.infoLabel, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>{label}</Text>
      <Text style={[styles.infoValue, { color: highlight ? ACCENT.green : colors.foreground, fontFamily: highlight ? "Inter_700Bold" : "Inter_500Medium" }]}>{value}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  shareBtn: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8 },
  shareBtnText: { fontSize: 14 },
  content: { padding: 16 },
  invoice: { borderWidth: 1, borderRadius: 12, overflow: "hidden" },
  letterhead: { padding: 20, flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  brandName: { fontSize: 22 },
  brandTagline: { fontSize: 12, marginTop: 2 },
  paidStamp: { borderWidth: 2, borderRadius: 6, paddingHorizontal: 12, paddingVertical: 6 },
  paidText: { fontSize: 14, letterSpacing: 1 },
  invoiceMeta: { flexDirection: "row", justifyContent: "space-between", padding: 16, borderBottomWidth: 1 },
  metaLabel: { fontSize: 10, letterSpacing: 0.8 },
  metaValue: { fontSize: 15 },
  partiesRow: { flexDirection: "row", padding: 16, gap: 16, borderBottomWidth: 1 },
  ...
  lineItem: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", gap: 8 },
