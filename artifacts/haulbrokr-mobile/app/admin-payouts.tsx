    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          ...
          {isAdmin && (
            <LastUpdated
              timestamp={payouts.dataUpdatedAt || undefined}
              style={{ marginTop: 2 }}
            />
          )}
        </View>
...
function StuckPayoutCard({
  ...
  const release = async () => {
    ...
    try {
      ...
      if (res.outcome === "released") {
        ...
        setResult({
          kind: "released",
          message: `${item.providerCompany} has been paid.`,
        });
      } else {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning);
        setResult({
          kind: "skipped",
          message: res.message || "Payout not eligible for release yet.",
        });
      }
    } catch (err: any) {
  ...
  const reset = async () => {
    ...
    try {
      ...
      setResult({
        kind: "reset",
        message: "Failure count and alert cleared — auto-retry re-armed.",
      });
    } catch (err: any) {
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
      setResult({
        ...
        message:
          err?.message ??
          "Couldn't clear the failure count. Try again shortly.",
      });
    }
  };
  ...
  return (
    ...
    >
      ...
      <View style={styles.statsRow}>
        <Stat
          label="Provider Net"
          value={money(item.providerNetAmount)}
          colors={colors}
        />
        <Stat
          label="Customer Paid"
          value={money(item.customerTotalAmount)}
          colors={colors}
        />
        <Stat
          label="Attempts"
          value={String(item.paymentAttempts)}
          colors={colors}
        />
        <Stat
          label="Retry Fails"
      ...
      {banner && (
        ...
        >
          ...
          <Text
            style={[
              styles.bannerText,
              { color: banner.color, fontFamily: "Inter_500Medium" },
            ]}
          >
            {result!.kind === "released"
              ? "Payout released — "
      ...
      {!released && (
        <>
          ...
          >
            The customer's payment already went through — only the provider
            transfer is pending. Releasing retries the transfer; the customer is
            never re-charged. Reset Failures clears a false alert and re-arms
            auto-retry once you've fixed the underlying problem.
          </Text>
            ...
            <Pressable
              ...
              style={[
                styles.releaseBtn,
                {
                  ...
                  opacity: retry.isPending || resetFailures.isPending ? 0.6 : 1,
                },
              ]}
            >
              {retry.isPending ? (
