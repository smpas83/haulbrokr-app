...
export default function TrackingScreen() {
  ...
  const { data: trackingData } = useJobTracking(
    isLiveJob && numericId != null ? numericId : null,
  );
  const hasLiveGps = !!(trackingData as any)?.latest?.lat;
  const gpsUpdatedAt = (trackingData as any)?.latest?.at as string | undefined;
  const job = liveJob
  ...
  const areaLabel = (() => {
    ...
    return state
      ? `${city.toUpperCase()} – ${state.toUpperCase()} AREA`
      : `${city.toUpperCase()} AREA`;
  })();
  ...
  const liveStatus =
    (trackingData as any)?.status ?? job?.status ?? "in_progress";
  const initialProgress = hasLiveGps
    ? (statusProgress[liveStatus] ?? 0.45)
    : 0.28;
  const progress = useRef(new Animated.Value(initialProgress)).current;
  const [progressPct, setProgressPct] = useState(
    Math.round(initialProgress * 100),
  );
  const [eta, setEta] = useState(hasLiveGps ? 0 : 34);

  useEffect(() => {
    if (hasLiveGps) {
      const target = statusProgress[liveStatus] ?? 0.45;
      Animated.timing(progress, {
        toValue: target,
        duration: 800,
        useNativeDriver: false,
      }).start();
      setProgressPct(Math.round(target * 100));
      setEta(0);
      return;
    }
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          ...
          >
            {hasLiveGps ? "● Live GPS" : "● In Progress"}
          </Text>
        </View>
        <View style={[styles.etaBadge, { backgroundColor: colors.primary }]}>
          ...
          >
            {hasLiveGps ? (gpsUpdatedAt ? "Live" : "Active") : `${eta} min`}
          </Text>
        </View>
      </View>
      ...
      >
        {/* Route map — live GPS when available, stylized fallback otherwise */}
        <View style={[styles.mapBox, { backgroundColor: "#0a1628" }]}>
          {hasLiveGps && (
            <View
              style={{
                ...
                right: 10,
                zIndex: 2,
                backgroundColor: ACCENT.green + "cc",
                paddingHorizontal: 8,
                paddingVertical: 4,
                borderRadius: 6,
              }}
          ...
          {[25, 50, 75].map((p) => (
            <View
              key={`h${p}`}
              style={[styles.roadH, { top: `${p}%` as any }]}
            />
          ))}
          {[25, 50, 75].map((p) => (
            <View
              key={`v${p}`}
              style={[styles.roadV, { left: `${p}%` as any }]}
            />
          ))}
          ...
          <View style={[styles.mapMarker, { right: "2%", top: "42%" }]}>
            <View
              style={[styles.markerDot, { backgroundColor: ACCENT.green }]}
            />
            <View
              style={[
                styles.markerLabel,
          ...
          <View style={[styles.etaOverlay, { backgroundColor: "#0a162890" }]}>
            ...
            <Text
              style={[
                styles.etaOverlayEta,
                { color: "#ffffff99", fontFamily: "Inter_400Regular" },
              ]}
            >
              ~{eta} min to delivery
            </Text>
