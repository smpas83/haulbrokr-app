...
import {
  useAdminAccess, useAdminCompliance, useReviewCompliance,
...
export default function AdminComplianceScreen() {
  ...
  const pending = items.filter((i) => i.status === "pending" || i.status === "not_submitted").length;
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { backgroundColor: colors.background, borderBottomColor: colors.border, paddingTop: topPad + 12 }]}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[styles.title, { color: colors.foreground, fontFamily: "Inter_700Bold" }]}>Carrier Review</Text>
...
function ComplianceCard({ item, colors }: { item: AdminComplianceItem; colors: any }) {
  ...
  return (
    <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      <View style={styles.fieldGrid}>
        ...
        <Field label="CDL State / Class" value={[item.cdlState, item.cdlClass].filter(Boolean).join(" · ") || null} colors={colors} />
        <Field label="FMCSA Authority" value={item.fmcsaAuthority} colors={colors} />
        <Field label="Insurance" value={item.insuranceActive} colors={colors} />
        <Field label="Operating Status" value={item.dotOperatingStatus} colors={colors} />
        <Field label="Not Suspended" value={item.notSuspended} colors={colors} />
      </View>
...
function Field({ label, value, colors }: { label: string; value?: string | number | null; colors: any }) {
...
export function ReviewBadge({ status, colors }: { status: string; colors: any }) {
  let bg = colors.muted, fg = colors.mutedForeground, icon: any = "info", label = status.replace(/_/g, " ");
  if (status === "verified" || status === "approved") {
    bg = ACCENT.green + "18"; fg = ACCENT.green; icon = "check-circle"; label = status === "verified" ? "Verified" : "Approved";
  } else if (status === "pending") {
    bg = "#f59e0b18"; fg = "#f59e0b"; icon = "clock"; label = "Pending";
  } else if (status === "rejected") {
    bg = colors.destructive + "18"; fg = colors.destructive; icon = "x-circle"; label = "Rejected";
  }
  return (
    <View style={[styles.badge, { backgroundColor: bg }]}>
      <Feather name={icon} size={11} color={fg} />
      <Text style={[styles.badgeText, { color: fg, fontFamily: "Inter_600SemiBold" }]}>{label}</Text>
    </View>
  );
}

export function ReviewActions({
  approveLabel, approveIcon, approveDisabled, rejectDisabled,
  reviewNote, status, isPending, colors, onSubmit,
}: {
  approveLabel: string;
  approveIcon: any;
  ...
  return (
    <View style={[styles.actions, { borderTopColor: colors.border }]}>
      {reviewNote && status === "rejected" && (
        <View style={[styles.noteBox, { backgroundColor: colors.destructive + "12", borderColor: colors.destructive + "33" }]}>
          <Text style={[styles.noteText, { color: colors.destructive, fontFamily: "Inter_400Regular" }]}>
            <Text style={{ fontFamily: "Inter_700Bold" }}>Rejection reason: </Text>{reviewNote}
          </Text>
        </View>
      )}
      {error && (
        <View style={[styles.noteBox, { backgroundColor: colors.destructive + "12", borderColor: colors.destructive + "33" }]}>
          <Text style={[styles.noteText, { color: colors.destructive, fontFamily: "Inter_500Medium" }]}>{error}</Text>
        </View>
      )}
      {rejecting ? (
        <View style={{ gap: 10 }}>
          ...
          <View style={styles.btnRow}>
            <Pressable
              ...
              style={[styles.btn, styles.rejectBtn, { borderColor: colors.destructive, opacity: isPending || !note.trim() ? 0.5 : 1 }]}
            >
              {isPending ? (
                ...
                <>
                  ...
                  <Text style={[styles.btnText, { color: colors.destructive, fontFamily: "Inter_700Bold" }]}>Confirm</Text>
                </>
              )}
            </Pressable>
            <Pressable
              onPress={() => { setRejecting(false); setNote(""); setError(null); }}
              disabled={isPending}
        ...
        <View style={styles.btnRow}>
          <Pressable
            ...
            style={[styles.btn, styles.approveBtn, { backgroundColor: colors.primary, opacity: isPending || approveDisabled ? 0.5 : 1 }]}
          >
            {isPending ? (
              <ActivityIndicator color={colors.primaryForeground} size="small" />
            ) : (
              <>
                <Feather name={approveIcon} size={15} color={colors.primaryForeground} />
                <Text style={[styles.btnText, { color: colors.primaryForeground, fontFamily: "Inter_700Bold" }]}>{approveLabel}</Text>
              </>
            )}
          </Pressable>
          <Pressable
            ...
            style={[styles.btn, styles.rejectBtn, { borderColor: colors.destructive, opacity: isPending || rejectDisabled ? 0.5 : 1 }]}
          >
            <Feather name="alert-circle" size={15} color={colors.destructive} />
            <Text style={[styles.btnText, { color: colors.destructive, fontFamily: "Inter_700Bold" }]}>Reject</Text>
          </Pressable>
        </View>
      )}
    </View>
...
export const styles = StyleSheet.create({
  ...
  noteBox: { borderWidth: 1, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8 },
  noteText: { fontSize: 12, lineHeight: 17 },
  input: { borderWidth: 1, borderRadius: 8, padding: 10, fontSize: 13, minHeight: 72, textAlignVertical: "top" },
  btnRow: { flexDirection: "row", gap: 10 },
  btn: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7, paddingVertical: 12, borderRadius: 8 },
  approveBtn: {},
