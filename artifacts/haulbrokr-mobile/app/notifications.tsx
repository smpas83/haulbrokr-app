...
type NotifItem = ActivityView & { createdAt: string; relatedId: number | null; read: boolean };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.background, borderBottomColor: colors.border, paddingTop: topPad + 12 }]}>
        ...
        {unread > 0 && (
          <Pressable onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); markAllRead(); }}>
      ...
      >
        ...
        {grouped.map((group, gi) => (
          <Animated.View key={group.label} entering={FadeInDown.delay(gi * 60).springify()}>
                ...
                return (
                  <View key={notif.id}>
                    <Pressable
                      ...
                      disabled={!tappable}
                      style={[styles.notifRow, { backgroundColor: notif.read ? "transparent" : ic + "08" }]}
                    >
                      {!notif.read && <View style={[styles.unreadDot, { backgroundColor: ic }]} />}
                      {notif.read && <View style={styles.unreadDotPlaceholder} />}

                      <View style={[styles.notifIcon, { backgroundColor: ic + "18" }]}>
                        <Feather name={notif.icon as any} size={16} color={ic} />
                      </View>

                      <View style={{ flex: 1, gap: 3 }}>
                        <Text style={[styles.notifTitle, { color: notif.read ? colors.mutedForeground : colors.foreground, fontFamily: notif.read ? "Inter_400Regular" : "Inter_600SemiBold" }]}>
                          {notif.text}
                        </Text>
                        <View style={styles.notifMeta}>
                          ...
                          {tappable && (
                            <Text style={[styles.notifLink, { color: ic, fontFamily: "Inter_500Medium" }]}>
...
const styles = StyleSheet.create({
  ...
  refreshPill: { flexDirection: "row", alignItems: "center", alignSelf: "center", gap: 8, paddingHorizontal: 12, paddingVertical: 6, borderWidth: 1, borderRadius: 999, marginBottom: 4 },
  refreshPillText: { fontSize: 12 },
  groupLabel: { fontSize: 10, letterSpacing: 1.2, marginBottom: 8, marginTop: 4 },
  groupCard: { borderWidth: 1, borderRadius: 12, overflow: "hidden", marginBottom: 20 },
  notifRow: { flexDirection: "row", alignItems: "flex-start", gap: 12, padding: 14, paddingLeft: 10 },
  ...
  empty: { alignItems: "center", justifyContent: "center", gap: 16, paddingTop: 80 },
