...
const DOC_SPECS: DocSpec[] = [
  // ── Driver hiring packet ───────────────────────────────────────────────
  {
    ...
    description: "Photo ID, front side",
    icon: "credit-card",
    required: true,
    expires: true,
    roles: ["driver"],
  },
  {
    ...
    description: "Photo ID, back side",
    icon: "credit-card",
    required: true,
    expires: true,
    roles: ["driver"],
  },
  {
    id: "cdl_front",
    label: "CDL — Front",
    description: "Commercial Driver's License, front",
    icon: "truck",
    required: true,
    expires: true,
    roles: ["driver"],
  },
  {
    id: "cdl_back",
    label: "CDL — Back",
    description: "CDL endorsements, back",
    icon: "truck",
    required: true,
    expires: true,
    roles: ["driver"],
  },
  {
    ...
    description: "Medical Examiner's Certificate",
    icon: "activity",
    required: true,
    expires: true,
    roles: ["driver"],
  },
  {
    ...
    description: "Most recent DOT drug screen result",
    icon: "alert-circle",
    required: true,
    roles: ["driver"],
  },
  {
    ...
    description: "Motor Vehicle Record — last 3 yrs",
    icon: "list",
    required: true,
    roles: ["driver"],
  },
  {
    ...
    description: "For payroll & I-9 verification",
    icon: "user",
    required: true,
    roles: ["driver"],
  },
  ...
  },
  {
    id: "twic",
    label: "TWIC Card",
    description: "Optional — port / secure-site access",
    icon: "key",
    required: false,
    expires: true,
    roles: ["driver"],
  ...
  // ── Carrier (provider company) documents ───────────────────────────────
  {
    ...
    description: "State / city business registration",
    icon: "briefcase",
    required: true,
    roles: ["provider"],
  },
  {
    id: "mc_authority",
    label: "MC Authority",
    description: "FMCSA operating authority letter",
    icon: "award",
    required: true,
    roles: ["provider"],
  },
  {
    ...
    description: "Registration for trucks in your fleet",
    icon: "file-text",
    required: true,
    expires: true,
    roles: ["provider"],
  },
  {
    ...
    description: "Itemized list of trucks & machines",
    icon: "list",
    required: true,
    roles: ["provider"],
  },
  {
    id: "signed_carrier_agreement",
    label: "Signed Carrier Agreement",
    description: "HaulBrokr carrier contract",
    icon: "edit-3",
    required: true,
    roles: ["provider"],
  },
  {
    ...
    description: "Auto liability & cargo proof",
    icon: "shield",
    required: true,
    expires: true,
  ...
  },
  {
    id: "w9",
    label: "W-9 Tax Form",
    description: "Signed W-9 for payments",
    icon: "file-text",
    required: true,
    roles: ["driver", "provider"],
  },
  {
    id: "dot_authority",
    label: "DOT Authority Letter",
    description: "USDOT / MC authority on file",
    icon: "award",
    required: false,
    roles: ["driver", "provider"],
  },
  {
    id: "voided_check",
    label: "Voided Check",
    description: "For ACH payout setup",
    icon: "credit-card",
    required: true,
    roles: ["provider"],
  },
  {
    ...
    description: "Signed direct-deposit authorization",
    icon: "edit-3",
    required: false,
    roles: ["provider"],
  },
  {
    id: "safety_rating",
    label: "Safety Rating",
    description: "FMCSA safety rating documentation",
    icon: "shield",
    required: false,
    roles: ["provider"],
  },
  {
    id: "bond",
    label: "Surety Bond",
    description: "Broker / carrier bond, if applicable",
    icon: "lock",
    required: false,
    roles: ["provider"],
  },
  // ── Customer documents ─────────────────────────────────────────────────
  {
    id: "po_template",
    label: "PO Template",
    description: "Your purchase-order template",
    icon: "file-text",
    required: false,
    roles: ["customer"],
  },
  {
    ...
    description: "Resale / exemption certificate, if applicable",
    icon: "percent",
    required: false,
    roles: ["customer"],
  },
...
const NUM_FIELDS: Partial<
  Record<DocId, { label: string; placeholder: string }>
> = {
...
function backendToUi(rows: RemoteDriverDoc[]): Record<DocId, DocRecord> {
  ...
  for (const [uiId, beType] of Object.entries(DOC_TYPE_MAP))
    reverse[beType] = uiId as DocId;
  for (const row of rows) {
    ...
    out[uiId] = {
      ...
      expiry: row.expiry
        ? new Date(row.expiry).toLocaleDateString("en-US")
        : undefined,
    };
  }
  return out;
...
export default function DriverDocsScreen() {
  ...
  const [docs, setDocs] = useState<Record<DocId, DocRecord>>(() =>
    emptyState(),
  );
  ...
  useEffect(() => {
    ...
    setDocs((prev) => {
      ...
      for (const key of dirtyRef.current) {
        ...
        if (merged[id])
          merged[id] = { ...merged[id], [field]: prev[id]?.[field] };
      }
      return merged;
    });
  }, [remote.data]);
  ...
  const handlePick = async (docId: DocId, source: "camera" | "library") => {
    ...
    try {
      ...
      const { objectPath, storageToken } = await upload.mutateAsync({
        uri: asset.uri,
        name: filename,
        mimeType,
      });
  ...
  const commitField = (
    ...
    if (key === "expiry") {
      ...
      if (value && !iso) {
        Alert.alert(
          "Date format",
          "Please enter the expiration as MM/DD/YYYY.",
        );
        return;
      }
      upsert.mutate(
  ...
  const roleSpecs = useMemo(
    () => DOC_SPECS.filter((d) => d.roles.includes(role)),
    [role],
  );
  const requiredSpecs = useMemo(
    () => roleSpecs.filter((d) => d.required),
    [roleSpecs],
  );
  ...
  const headerCopy =
    role === "provider"
      ? {
          ...
          subtitle: "Carrier compliance & payout paperwork on file",
          packet: "Carrier packet",
          section: "REQUIRED TO ONBOARD",
        }
      : role === "customer"
        ? {
            title: "Billing Documents",
            subtitle: "Optional paperwork to speed up invoicing",
            packet: "Billing packet",
            section: "DOCUMENTS",
          }
        : {
            title: "Driver Documents",
            subtitle:
              "Everything you need on file to get hired and start hauling",
            packet: "Hiring packet",
            section: "REQUIRED FOR HIRING",
          };
    ...
    <>
      ...
      <ScrollView
        ...
        contentContainerStyle={[
          styles.content,
          { paddingTop: topPad, paddingBottom: 60 + insets.bottom },
        ]}
      ...
      >
        ...
        <View
          style={[
            styles.progressCard,
            {
              ...
              borderColor: allRequired ? "#16a34a40" : colors.border,
            },
          ]}
        >
          <View style={styles.progressHeader}>
            ...
            <Text
              style={[
                styles.progressPct,
                {
                  color: allRequired ? "#16a34a" : colors.primary,
          ...
          >
            <View
              style={[
                styles.progressFill,
                {
                  backgroundColor: allRequired ? "#16a34a" : colors.primary,
                  width: `${pct}%` as any,
                },
              ]}
            />
        ...
        >
          Documents you upload here are visible to your dispatcher and the
          customer site supervisor for active jobs. Keep expirations current to
    ...
    <Pressable
      ...
      style={[
        styles.docCard,
        {
          backgroundColor: colors.card,
          borderColor:
            status === "missing" && spec.required ? "#ef444430" : colors.border,
        },
      ]}
    >
        ...
        <View style={[styles.docBody, { borderTopColor: colors.border }]}>
          ...
          <View style={styles.actionRow}>
            <Pressable
              ...
              style={[
                styles.actionBtn,
                { backgroundColor: colors.primary, opacity: isBusy ? 0.6 : 1 },
              ]}
            >
              {isBusy ? (
                <ActivityIndicator
