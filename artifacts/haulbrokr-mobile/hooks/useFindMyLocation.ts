    ...
    async (opts?: { follow?: boolean }) => {
      ...
      try {
        ...
        if (!servicesEnabled) {
          ...
          setError(
            "Location services are turned off on this device. Enable them in Settings to use Find My Location.",
          );
          return null;
        }
        ...
        if (status !== "granted") {
          ...
          setError(
            "Location permission was denied. Enable location access in Settings, then tap Retry.",
          );
          return null;
        }
