...
export function useUpdateBid() {
  ...
  return useMutation({
    mutationFn: (data: {
      ...
      const { id, ...body } = data;
      return apiFetch(getToken, "PATCH", `/bids/${id}`, body);
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["live-jobs"] });
      qc.invalidateQueries({ queryKey: ["live-requests"] });
    },
  });
}
...
export function useMarketplaceQuote() {
  ...
  return useMutation({
    mutationFn: (data: {
      ...
      materialType?: string;
      quantityTons?: number;
    }) => apiFetch(getToken, "POST", "/marketplace/quotes", data),
  });
}

export function useDispatchRecommendations(jobId?: number | null) {
  const { getToken, isSignedIn } = useAuth();
  return useQuery<{
    jobId: number;
    recommendations: {
    ...
    queryFn: () =>
      apiFetch(getToken, "GET", `/jobs/${jobId}/dispatch-recommendations`),
    enabled: !!isSignedIn && !!jobId,
  });
}
...
export function useAssignJob() {
  ...
  return useMutation({
    mutationFn: (data: {
      ...
      const { id, ...body } = data;
      return apiFetch(getToken, "POST", `/jobs/${id}/assign`, body);
    },
    onSuccess: (_data, vars) => {
      qc.invalidateQueries({ queryKey: ["live-jobs"] });
      qc.invalidateQueries({ queryKey: ["tickets", vars.id] });
      qc.invalidateQueries({ queryKey: ["dispatch-recommendations", vars.id] });
    },
  });
}
