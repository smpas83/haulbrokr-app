...
function buildProfileFields(body: any): Record<string, unknown> {
  ...
  if (
    ...
    VALID_PAYMENT_TERMS.includes(body.paymentTerms)
  ) {
...
router.patch("/profiles/me", requireAuth, async (req, res): Promise<void> => {
  ...
  if (req.body?.companyName !== undefined)
    allowed.companyName = String(req.body.companyName).trim();
...
router.post("/profiles", requireAuth, async (req, res): Promise<void> => {
  ...
  const inviteCode = req.body?.inviteCode
    ? String(req.body.inviteCode).trim().toUpperCase()
    : undefined;
  ...
  if (role === "customer" || role === "provider") {
    ...
    await db
      ...
      .where(eq(profilesTable.id, newProfile.id));
    res
      .status(201)
      .json({ ...newProfile, organizationId: org.id, organization: org });
    return;
  }
  ...
  if (!inviteCode) {
    res.status(400).json({
      error: "An invite code is required to join as a driver or supervisor.",
    });
    return;
  }
  const [org] = await db
    .select()
    .from(organizationsTable)
    .where(eq(organizationsTable.inviteCode, inviteCode));
  if (!org) {
    res.status(404).json({
      error: "Invite code not found. Ask your manager to share it again.",
    });
    return;
  }
  ...
  if (role === "supervisor" && org.type !== "customer") {
    res.status(400).json({
      error: "Supervisors join a Customer (construction company) team.",
    });
    return;
  }
  if (role === "driver" && org.type !== "provider") {
    res
      .status(400)
      .json({ error: "Drivers join a Provider (trucking company) team." });
    return;
  }
  const [newProfile] = await db
    .insert(profilesTable)
