...
router.get("/account/status", requireProfile, async (req, res): Promise<void> => {
  ...
  res.json(GetAccountStatusResponse.parse({
    profileComplete,
    w9Status,
    insuranceStatus,
    paymentStatus,
...
router.post("/account/w9", requireProfile, async (req, res): Promise<void> => {
  ...
  const existing = await db.select().from(w9SubmissionsTable).where(eq(w9SubmissionsTable.profileId, profile.id));
  if (existing.length > 0) {
    res.status(409).json({ error: "W-9 already submitted. Use PATCH to update." });
    return;
  }
  const [w9] = await db.insert(w9SubmissionsTable).values({
    ...parsed.data,
...
router.get("/account/insurance", requireProfile, async (req, res): Promise<void> => {
  ...
  res.json(GetInsuranceResponse.parse({
    ...insurance,
    glCoverageAmount: parseFloat(insurance.glCoverageAmount),
    autoCoverageAmount: insurance.autoCoverageAmount ? parseFloat(insurance.autoCoverageAmount) : null,
    bondAmount: insurance.bondAmount ? parseFloat(insurance.bondAmount) : null,
  }));
});
...
router.post("/account/insurance", requireProfile, async (req, res): Promise<void> => {
  ...
  const existing = await db.select().from(insuranceSubmissionsTable).where(eq(insuranceSubmissionsTable.profileId, profile.id));
  if (existing.length > 0) {
    res.status(409).json({ error: "Insurance already submitted. Use PATCH to update." });
    return;
  }
  const [insurance] = await db.insert(insuranceSubmissionsTable).values({
    ...parsed.data,
    profileId: profile.id,
    status: "pending",
    glCoverageAmount: String(parsed.data.glCoverageAmount),
    autoCoverageAmount: parsed.data.autoCoverageAmount != null ? String(parsed.data.autoCoverageAmount) : undefined,
    bondAmount: parsed.data.bondAmount != null ? String(parsed.data.bondAmount) : undefined,
  }).returning();
  res.status(201).json(GetInsuranceResponse.parse({
    ...insurance,
    glCoverageAmount: parseFloat(insurance.glCoverageAmount),
    autoCoverageAmount: insurance.autoCoverageAmount ? parseFloat(insurance.autoCoverageAmount) : null,
    bondAmount: insurance.bondAmount ? parseFloat(insurance.bondAmount) : null,
...
async function ensureStripeCustomerId(profile: any): Promise<string> {
  ...
  await db.update(profilesTable).set({ stripeCustomerId: customer.id }).where(eq(profilesTable.id, profile.id));
...
async function resolveStripePaymentMethod(
  ...
  const attachedTo = typeof pmObj.customer === "string" ? pmObj.customer : pmObj.customer?.id ?? null;
  if (attachedTo && attachedTo !== customerId) {
    throw Object.assign(new Error("Payment method does not belong to this account."), { status: 400 });
  }
  if (!attachedTo) {
  ...
  if (bank) {
    ...
    if (!setupIntentId) {
      throw Object.assign(new Error("Connect your bank account through the secure setup flow."), { status: 400 });
    }
    const si = await stripe.setupIntents.retrieve(setupIntentId);
    const siCustomer = typeof si.customer === "string" ? si.customer : si.customer?.id ?? null;
    if (siCustomer && siCustomer !== customerId) {
      throw Object.assign(new Error("Setup intent does not belong to this account."), { status: 400 });
    }
    const siPm = typeof si.payment_method === "string" ? si.payment_method : si.payment_method?.id ?? null;
    if (siPm !== pmObj.id) {
      throw Object.assign(new Error("Setup intent does not match this bank account."), { status: 400 });
    }
...
router.post("/account/payment-method/setup-intent", requireProfile, async (req, res): Promise<void> => {
  ...
  try {
    ...
    res.json(CreatePaymentMethodSetupIntentResponse.parse({ clientSecret: intent.client_secret, publishableKey }));
  } catch (err: any) {
    req.log?.error?.({ err }, "Failed to create SetupIntent");
    res.status(502).json({ error: err?.message ?? "Could not start card setup" });
  }
});
...
router.post("/account/payment-method/bank-setup-intent", requireProfile, async (req, res): Promise<void> => {
  ...
  try {
    ...
    res.status(502).json({ error: err?.message ?? "Could not start bank setup" });
  }
});
...
router.post("/account/payment-method", requireProfile, async (req, res): Promise<void> => {
  ...
  if (NET_TERMS.includes(parsed.data.methodType)) {
    const [creditApp] = await db.select().from(creditApplicationsTable).where(eq(creditApplicationsTable.profileId, profile.id));
    if (!creditApp || creditApp.status !== "approved") {
      res.status(403).json({ error: "Net invoicing terms require an approved credit application." });
      return;
    }
  }
  const existing = await db.select().from(paymentMethodsTable).where(eq(paymentMethodsTable.profileId, profile.id));
  if (existing.length > 0) {
    res.status(409).json({ error: "Payment method already set. Use PATCH to update." });
    return;
  }
  let stripeFields: ResolvedInstrumentFields | undefined;
  if (parsed.data.stripePaymentMethodId) {
    try {
      stripeFields = await resolveStripePaymentMethod(profile, parsed.data.stripePaymentMethodId, parsed.data.stripeSetupIntentId);
    } catch (err: any) {
      res.status(err?.status ?? 502).json({ error: err?.message ?? "Could not save card" });
      return;
    }
  }
...
router.patch("/account/payment-method", requireProfile, async (req, res): Promise<void> => {
  ...
  const NET_TERMS = ["net_15", "net_30", "net_45"];
  if (parsed.data.methodType != null && NET_TERMS.includes(parsed.data.methodType)) {
    const [creditApp] = await db.select().from(creditApplicationsTable).where(eq(creditApplicationsTable.profileId, profile.id));
    if (!creditApp || creditApp.status !== "approved") {
      res.status(403).json({ error: "Net invoicing terms require an approved credit application." });
      return;
...
router.post("/account/payment-method/verify-microdeposits", requireProfile, async (req, res): Promise<void> => {
  ...
  const hasAmounts = Array.isArray(parsed.data.amounts) && parsed.data.amounts.length === 2;
  const hasCode = typeof parsed.data.descriptorCode === "string" && parsed.data.descriptorCode.length > 0;
  if (hasAmounts === hasCode) {
    res.status(400).json({ error: "Provide either the two deposit amounts or the descriptor code, not both." });
    return;
  }

  const [pm] = await db.select().from(paymentMethodsTable).where(eq(paymentMethodsTable.profileId, profile.id));
  if (!pm || pm.methodType !== "ach" || !pm.stripeSetupIntentId || pm.verificationStatus !== "pending") {
    res.status(404).json({ error: "No bank account is awaiting verification." });
    return;
  }
  ...
  try {
    si = await stripe.setupIntents.verifyMicrodeposits(
      ...
      hasAmounts ? { amounts: parsed.data.amounts } : { descriptor_code: parsed.data.descriptorCode },
    );
  } catch (err: any) {
    // Stripe returns a 400-class error for wrong amounts/code or too many attempts.
    res.status(400).json({ error: err?.message ?? "Verification failed. Double-check the amounts and try again." });
    return;
  }

  if (si.status !== "succeeded") {
    res.status(400).json({ error: "Verification could not be completed. Please try again." });
    return;
  }
...
router.post("/account/payout", requireProfile, async (req, res): Promise<void> => {
  ...
  const existing = await db.select().from(payoutAccountsTable).where(eq(payoutAccountsTable.profileId, profile.id));
  if (existing.length > 0) {
    res.status(409).json({ error: "Payout account already set. Use PATCH to update." });
    return;
  }
  ...
  const [payout] = await db.insert(payoutAccountsTable).values({
    ...rest,
    routingLast4,
    accountLast4,
    profileId: profile.id,
...
router.patch("/account/compliance/verify", requireProfile, async (req, res): Promise<void> => {
  ...
  if (!rec) {
    res.status(404).json({ error: "No compliance record to verify. Submit DOT/CDL details first." });
    return;
  }
  res.json(rec);
});
...
router.post("/account/credit-application", requireProfile, async (req, res): Promise<void> => {
  ...
  const values: Record<string, unknown> = {
    ...
    estimatedMonthlySpend: d.estimatedMonthlySpend != null ? String(d.estimatedMonthlySpend) : undefined,
    status: "pending",
  };
  const [existing] = await db.select().from(creditApplicationsTable).where(eq(creditApplicationsTable.profileId, profile.id));
  let rec;
  if (existing) {
    [rec] = await db.update(creditApplicationsTable).set(values).where(eq(creditApplicationsTable.profileId, profile.id)).returning();
  } else {
    [rec] = await db.insert(creditApplicationsTable).values({ ...values, profileId: profile.id }).returning();
  }
  res.json({ ...rec, estimatedMonthlySpend: rec.estimatedMonthlySpend ? parseFloat(rec.estimatedMonthlySpend) : null });
});
