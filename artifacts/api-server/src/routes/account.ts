    ...
    if (NET_TERMS.includes(parsed.data.methodType)) {
      ...
      if (!creditApp || creditApp.status !== "approved") {
        res
          ...
          .json({
            error:
              "Net invoicing terms require an approved credit application.",
          });
        return;
      }
    }
  ...
  async (req, res): Promise<void> => {
    ...
    if (!rec) {
      res
        ...
        .json({
          error:
            "No compliance record to verify. Submit DOT/CDL details first.",
        });
      return;
    }
    res.json(rec);
 ...
 */
router.post(
  "/account/compliance/fmcsa-lookup",
  requireProfile,
  async (req, res): Promise<void> => {
    const profile = getRequestProfile(req);
    const body = req.body ?? {};
    const dotNumber =
      typeof body.dotNumber === "string" ? body.dotNumber.trim() : "";
    const mcNumber =
      typeof body.mcNumber === "string" ? body.mcNumber.trim() : "";
    ...
    if (!isFmcsaConfigured()) {
      res.status(503).json({
        error:
          "FMCSA_WEB_KEY is not configured. Live carrier lookup is unavailable.",
    ...
    const snapshot = deriveComplianceFromFmcsa(
      ...
      authOk
        ? {
            commonAuthority: authOk.commonAuthority,
            contractAuthority: authOk.contractAuthority,
          }
        : null,
    );
    ...
    res.json({
      ...
      carrier: {
        ...
        commonAuthorityStatus:
          authOk?.commonAuthority ?? carrier.commonAuthorityStatus,
        contractAuthorityStatus:
          authOk?.contractAuthority ?? carrier.contractAuthorityStatus,
      },
      snapshot,
    });
  },
