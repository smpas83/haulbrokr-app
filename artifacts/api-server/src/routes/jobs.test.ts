 ...
 */
function mockStripeFailure(
  failAt: "paymentIntent" | "transfer",
 ...
 */
function mockStripeAuthRequired(
  pi = {
    id: "pi_auth_1",
    client_secret: "pi_auth_1_secret",
...
describe("POST /jobs/:id/charge", () => {
  for (const c of notReadyCases) {
    it(`returns 409 with the guard message when the provider is ${c.reason}`, async () => {
      ...
      h.rows.set(paymentMethodsTable, [
        { profileId: CUSTOMER_ID, methodType: "credit_card" },
      ]);
    ...
    h.rows.set(paymentMethodsTable, [
      {
        ...
        stripePaymentMethodId: "pm_real_123",
      },
    ]);
    h.rows.set(profilesTable, [
      { id: CUSTOMER_ID, stripeCustomerId: "cus_real_123" },
    ]);
    const stripe = mockStripe();
    ...
    h.rows.set(paymentMethodsTable, [
      {
        profileId: CUSTOMER_ID,
        methodType: "ach",
        stripePaymentMethodId: "pm_bank_real_123",
      },
    ]);
    h.rows.set(profilesTable, [
      { id: CUSTOMER_ID, stripeCustomerId: "cus_real_123" },
  ...
  it("creates a net-terms invoice without moving money (net_30)", async () => {
    h.rows.set(jobsTable, [baseJob()]);
    h.rows.set(paymentMethodsTable, [
      { profileId: CUSTOMER_ID, methodType: "net_30" },
    ]);
    h.rows.set(creditApplicationsTable, [
      { profileId: CUSTOMER_ID, status: "approved" },
    ]);
    const stripe = mockStripe();
    ...
    const deltaMs =
      (invoiced!.paymentDueDate as Date).getTime() -
      (invoiced!.invoicedAt as Date).getTime();
    expect(deltaMs).toBe(30 * DAY_MS);
  });

  const chargeFailureCases = [
    {
      failAt: "paymentIntent" as const,
      label: "paymentIntents.create rejects",
    },
  ...
  it("retries an instant payment that previously failed and marks it released", async () => {
    ...
    h.rows.set(jobsTable, [
      baseJob({ paymentStatus: "failed", invoicedAt: null }),
    ]);
    h.rows.set(paymentMethodsTable, [
      { profileId: CUSTOMER_ID, methodType: "credit_card" },
    ]);
  ...
  it("refuses to re-charge a failed job that still has an open invoice", async () => {
    ...
    h.rows.set(jobsTable, [
      baseJob({ paymentStatus: "failed", invoicedAt: new Date() }),
    ]);
    h.rows.set(paymentMethodsTable, [
      { profileId: CUSTOMER_ID, methodType: "net_30" },
    ]);
  ...
  for (const c of dueDateCases) {
    it(`sets the invoice due date ${c.days} days out for ${c.methodType}`, async () => {
      h.rows.set(jobsTable, [baseJob()]);
      h.rows.set(paymentMethodsTable, [
        { profileId: CUSTOMER_ID, methodType: c.methodType },
      ]);
      h.rows.set(creditApplicationsTable, [
        { profileId: CUSTOMER_ID, status: "approved" },
      ]);
...
describe("POST /jobs/:id/release", () => {
    ...
    h.rows.set(paymentMethodsTable, [
      {
        profileId: CUSTOMER_ID,
        methodType: "credit_card",
        stripePaymentMethodId: "pm_real_456",
      },
    ]);
    h.rows.set(profilesTable, [
      { id: CUSTOMER_ID, stripeCustomerId: "cus_real_456" },
    ]);
    const stripe = mockStripe();
  ...
  it("retried release uses a fresh idempotency key tied to the new attempt", async () => {
    ...
    expect(res.status).toBe(200);
    expect(stripe.paymentIntents.create.mock.calls[0][1].idempotencyKey).toBe(
      `job-charge:${JOB_ID}:2`,
    );
    expect(stripe.transfers.create.mock.calls[0][1].idempotencyKey).toBe(
      `job-transfer:${JOB_ID}:2`,
    );
    const released = h.updates.find((u) => u.paymentStatus === "released");
    expect(released!.paymentAttempts).toBe(2);
  });
...
describe("GET /jobs/:id/payment-confirmation", () => {
  it("returns the client secret + publishable key for a job awaiting confirmation", async () => {
    ...
    const res = await request(makeApp()).get(
      `/jobs/${JOB_ID}/payment-confirmation`,
    );
...
describe("POST /jobs/:id/confirm-payment", () => {
  ...
  it("does NOT re-charge or mark failed when the payout transfer fails after a succeeded charge", async () => {
    ...
    const transfers = {
      create: vi.fn(async () => {
        throw new Error("Transfer is down");
      }),
    };
    vi.mocked(getUncachableStripeClient).mockResolvedValue({
      paymentIntents,
...
describe("computeBreakdown (15% broker-fee model)", () => {
  it("splits a clean amount into base, fee, and gross", () => {
    expect(computeBreakdown(100, 1, 0.15)).toEqual({
      base: 100,
      fee: 15,
      gross: 115,
    });
  ...
  it("keeps the provider net equal to the base (work value)", () => {
    ...
    expect(computeBreakdown(50, 2.5, 0.15)).toEqual({
      base: 125,
      fee: 18.75,
      gross: 143.75,
    });
  ...
  it("rounds the fee to cents (no sub-cent drift)", () => {
    ...
    expect(computeBreakdown(33.33, 3, 0.15)).toEqual({
      base: 99.99,
      fee: 15,
      gross: 114.99,
    });
  ...
  it("avoids floating-point artifacts in the gross", () => {
    ...
    expect(computeBreakdown(19.99, 1, 0.15)).toEqual({
      base: 19.99,
      fee: 3,
      gross: 22.99,
    });
...
function mockStripeCheckout(
  ...
  const sessions = {
    create: vi.fn(async (_args: any) => ({
      id: "cs_test_1",
      url: opts.createUrl ?? "https://checkout.stripe.com/c/pay/cs_test_1",
    })),
...
describe("POST /jobs/:id/checkout-session", () => {
  it("creates a destination-charge Checkout Session: gross line item, 15% application fee, transfer to provider", async () => {
    ...
    expect(decodeURIComponent(args.success_url)).toContain(
      "https://app.replit.dev/job/10",
    );
  ...
  it("returns 400 when the job is not completed", async () => {
    h.rows.set(jobsTable, [
      baseJob({ status: "in_progress", paymentStatus: "unpaid" }),
    ]);
    mockStripeCheckout();
  ...
  it("finalizes a paid session: records the PaymentIntent/transfer and marks the job released", async () => {
    ...
    mockStripeCheckout({
      retrieved: {
        ...
        payment_intent: {
          id: "pi_co_1",
          latest_charge: { id: "ch_co_1", transfer: "tr_co_1" },
        },
      },
    });
