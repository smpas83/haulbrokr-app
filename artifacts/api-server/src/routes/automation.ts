    ...
    try {
      const [
        ...
        db
          ...
          .where(sql`${w9SubmissionsTable.status} = 'pending'`),
        db
          .select({ count: sql<number>`count(*)` })
          .from(insuranceSubmissionsTable)
          .where(sql`${insuranceSubmissionsTable.status} = 'pending'`),
        db
          .select({ count: sql<number>`count(*)` })
          .from(driverDocumentsTable)
          .where(sql`${driverDocumentsTable.status} = 'missing'`),
        db
          .select({ count: sql<number>`count(*)` })
          .from(driverDocumentsTable)
          .where(sql`${driverDocumentsTable.status} = 'uploaded'`),
      ]);
