...
const InviteMemberBody = z.object({
  ...
  orgRole: z.enum(["admin", "member", "fleet_manager", "dispatcher"]).default("member"),
...
router.patch("/organizations/me", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!canManageCompany(profile)) {
    res.status(403).json({ error: "Only company managers can update company details." });
    return;
  }
...
router.get("/organizations/compliance-status", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!hasOrgPermission(profile, "view_compliance") && !isOrgManager(profile)) {
    res.status(403).json({ error: "You do not have permission to view compliance status." });
    return;
  }
  const [org] = await db
    .select()
...
router.get("/organizations/members", requireAuth, async (req, res): Promise<void> => {
  ...
  res.json({
    ...
    roster: {
      ...
      customers: members.filter((m) => m.role === "customer" || m.role === "supervisor"),
      dispatchers: members.filter((m) => m.orgRole === "dispatcher"),
      fleetManagers: members.filter((m) => m.orgRole === "fleet_manager" || m.orgRole === "owner"),
    },
  });
});

/** Roster filtered by audience: drivers, dispatchers, fleet managers, customers. */
router.get("/organizations/roster", requireAuth, async (req, res): Promise<void> => {
  const clerkId = req.clerkId as string;
  const profile = await loadProfile(clerkId);
  if (!profile?.organizationId) {
    res.status(404).json({ error: "No organization" });
  ...
  const customers = members.filter((m) => m.role === "customer" || m.role === "supervisor");
  const dispatchers = members.filter((m) => m.orgRole === "dispatcher");
  const fleetManagers = members.filter((m) => m.orgRole === "fleet_manager" || m.orgRole === "owner");
 ...
 */
router.post("/organizations/members/invite", requireAuth, async (req, res): Promise<void> => {
  const clerkId = req.clerkId as string;
  const profile = await loadProfile(clerkId);
  if (!profile?.organizationId) {
    res.status(404).json({ error: "No organization" });
  ...
  const [target] = await db.select().from(profilesTable).where(eq(profilesTable.clerkId, parsed.data.clerkId));
  if (!target) {
    res.status(404).json({ error: "No profile found for that Clerk user. They must sign up first." });
    return;
  }
  if (target.organizationId && target.organizationId !== profile.organizationId) {
    res.status(409).json({ error: "That user already belongs to another organization." });
    return;
  }
...
router.post("/organizations/rotate-code", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!canManageMembers(profile)) {
    res.status(403).json({ error: "Only managers can rotate the invite code." });
    return;
  }
