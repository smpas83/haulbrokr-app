...
import { getRequestProfile, requireAuth, requireProfile } from "../middlewares/requireAuth";
import {
...
function authAgeSeconds(req: { auth?: { sessionClaims?: Record<string, unknown> }; headers: Record<string, unknown> }): number | null {
...
router.get("/account/deletion/preview", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  const preview = await previewAccountDeletion(profile.id);
  res.json(preview);
});

router.get("/account/deletion", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  const [row] = await db
    .select()
 ...
 */
router.post("/account/deletion", requireAuth, requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  ...
  if (age == null || age > 600) {
    res.status(401).json({
      error: "Recent authentication required. Sign in again, then confirm deletion.",
...
router.post("/account/deletion/resume", requireAuth, requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  ...
  const [row] = await db
    .select()
    .from(accountDeletionRequestsTable)
    .where(and(eq(accountDeletionRequestsTable.id, id), eq(accountDeletionRequestsTable.profileId, profile.id)));
  if (!row) {
  ...
  try {
    ...
    res.status(500).json({ error: err instanceof Error ? err.message : "Resume failed", resumable: true });
  }
});
...
router.delete("/profiles/me", requireAuth, async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (req.headers["x-confirm-delete"] !== "DELETE") {
      res.status(400).json({
        error: 'Send header X-Confirm-Delete: DELETE and prefer POST /account/deletion with confirmation body.',
...
router.post("/account/export", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  try {
    ...
    res.status(500).json({ error: err instanceof Error ? err.message : "Export request failed" });
  }
});

router.get("/account/export", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  const rows = await listDataExports(profile.id);
  res.json(rows);
});

router.get("/account/export/:id", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
...
router.get("/account/export/:id/download", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  const id = Number(req.params.id);
  if (!Number.isFinite(id)) {
    res.status(400).json({ error: "Invalid export id" });
  ...
  try {
    ...
    logger.error({ err, exportId: id, profileId: profile.id }, "Export download failed");
