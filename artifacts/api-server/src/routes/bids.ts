...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const rawId = Array.isArray(req.params.requestId)
    ...
    let customerSideAllowed =
      profile.role === "customer" && request.customerId === profile.id;
    if (
      !customerSideAllowed &&
      (profile.role === "customer" || profile.role === "supervisor") &&
      profile.organizationId
    ) {
      const [customerProfile] = await db
        .select()
        .from(profilesTable)
        .where(eq(profilesTable.id, request.customerId));
      customerSideAllowed =
    ...
    if (customerSideAllowed) {
      bids = await db
        .select()
        .from(bidsTable)
        .where(eq(bidsTable.requestId, params.data.requestId));
    } else {
      bids = await db
        .select()
        .from(bidsTable)
        .where(
          and(
            eq(bidsTable.requestId, params.data.requestId),
            eq(bidsTable.providerId, profile.id),
          ),
        );
    }
    ...
    const enriched = await Promise.all(
      bids.map(async (b) => {
        const [provider] = await db
          .select()
          .from(profilesTable)
          .where(eq(profilesTable.id, b.providerId));
        return {
          ...b,
          ratePerHour: parseFloat(b.ratePerHour),
          estimatedHours: b.estimatedHours
            ? parseFloat(b.estimatedHours)
            : null,
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const rawId = Array.isArray(req.params.requestId)
      ? req.params.requestId[0]
      : req.params.requestId;
    const pathParams = CreateBidParams.safeParse({
      requestId: parseInt(rawId, 10),
    });
    ...
    const [request] = await db
      .select()
      .from(requestsTable)
      .where(eq(requestsTable.id, pathParams.data.requestId));
    if (!request) {
      res.status(404).json({ error: "Request not found" });
      return;
    }
...
router.get("/bids/:id", requireProfile, async (req, res): Promise<void> => {
  ...
  if (!allowed) {
    const [request] = await db
      .select()
      .from(requestsTable)
      .where(eq(requestsTable.id, bid.requestId));
    if (request) {
  ...
  const [provider] = await db
    .select()
    .from(profilesTable)
    .where(eq(profilesTable.id, bid.providerId));
  res.json(
    GetBidResponse.parse({
      ...bid,
      ratePerHour: parseFloat(bid.ratePerHour),
      estimatedHours: bid.estimatedHours
        ? parseFloat(bid.estimatedHours)
...
router.patch("/bids/:id", requireProfile, async (req, res): Promise<void> => {
  ...
  const [request] = await db
    .select()
    .from(requestsTable)
    .where(eq(requestsTable.id, existingBid.requestId));
  if (!request) {
    res.status(404).json({ error: "Request not found" });
    return;
  }
  ...
  if (parsed.data.status === "accepted" || parsed.data.status === "rejected") {
    if (!isCustomerRequestOwner) {
      res
        .status(403)
        .json({ error: "Only the request owner can accept or reject bids" });
      return;
    }
  ...
  if (parsed.data.status === "accepted") {
    ...
    if (job) {
      await recordMarketplaceAudit({
        ...
        action: "job.commission_snapshot",
        entityType: "job",
        entityId: job.id,
        after: {
          commission,
    ...
    await db
      ...
      .where(
        and(
          eq(bidsTable.requestId, request.id),
