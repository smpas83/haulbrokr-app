  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!checkUploadRateLimit(String(profile.id))) {
        res.status(429).json({
          error:
            "Too many upload requests. Please wait before requesting another upload URL.",
        });
        return;
      }
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (err instanceof ObjectNotFoundError) {
        res.status(422).json({
          error:
            "Uploaded object not found in storage — upload may not have completed",
        });
      } else {
    ...
    const normalizedDeclared = declaredContentType
      .split(";")[0]
      .trim()
      .toLowerCase();
    const normalizedActual = actualContentType
      .split(";")[0]
      .trim()
      .toLowerCase();
    if (
      ...
      try {
        ...
        req.log.warn(
          { err: delErr, objectPath },
          "Failed to delete content-type mismatch upload",
        );
      }
      res.status(422).json({
