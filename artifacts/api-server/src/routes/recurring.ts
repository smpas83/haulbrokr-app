...
import { db, recurringGenerationRunsTable, recurringSchedulesTable } from "@workspace/db";
...
router.get("/recurring-schedules", requireProfile, async (req, res): Promise<void> => {
...
router.post("/recurring-schedules", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  if (profile.role !== "customer" && profile.role !== "supervisor") {
    res.status(403).json({ error: "Only customers can create recurring haul schedules." });
    return;
  }
...
router.get("/recurring-schedules/:id", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  const id = Number(req.params.id);
  const [row] = await db
    .select()
    .from(recurringSchedulesTable)
    .where(and(eq(recurringSchedulesTable.id, id), eq(recurringSchedulesTable.customerId, profile.id)));
  if (!row) {
...
router.patch("/recurring-schedules/:id", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  const id = Number(req.params.id);
  ...
  if (status && ["active", "paused", "cancelled"].includes(status)) {
    const [row] = await db
      ...
      .set({ status: status as "active" | "paused" | "cancelled", lastError: null })
  ...
  for (const [k, v] of Object.entries(d)) {
    ...
    if (k === "startDate" || k === "endDate") patch[k] = v ? new Date(String(v)) : null;
    else if (k === "quantityTons" || k === "estimatedHours" || k === "budgetPerHour") {
...
router.post("/recurring-schedules/:id/pause", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  const id = Number(req.params.id);
  const [row] = await db
    .update(recurringSchedulesTable)
...
router.post("/recurring-schedules/:id/resume", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  const id = Number(req.params.id);
  const [row] = await db
    .update(recurringSchedulesTable)
...
router.post("/recurring-schedules/:id/cancel", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  const id = Number(req.params.id);
  const [row] = await db
    .update(recurringSchedulesTable)
