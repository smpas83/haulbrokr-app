...
const REQUIRED: Record<EventType, { gps: boolean; gpsConfirmed: boolean; files: string[] }> = {
  checkin:  { gps: true,  gpsConfirmed: false, files: ["selfie", "truck", "license_plate"] },
  checkout: { gps: true,  gpsConfirmed: false, files: [] },
  pickup:   { gps: false, gpsConfirmed: false, files: ["loaded_truck", "scale_ticket"] },
  delivery: { gps: false, gpsConfirmed: true,  files: ["delivered_material", "customer_signature"] },
};

const TIMELINE_STATUS: Record<EventType, "checked_in" | "loaded" | "completed" | "arrived"> = {
  checkin: "checked_in",
  checkout: "arrived",
  pickup: "loaded",
  delivery: "completed",
...
router.post("/jobs/:jobId/driver-events", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  const jobId = parseInt(req.params.jobId as string, 10);
  if (!Number.isFinite(jobId)) { res.status(400).json({ error: "Invalid job id" }); return; }

  if (!DRIVER_SIDE.has(profile.role)) {
    res.status(403).json({ error: "Only drivers and providers can submit driver events." });
    return;
  }
  ...
  if (!REQUIRED[eventType]) {
    res.status(400).json({ error: "Invalid eventType. Expected checkin|checkout|pickup|delivery." });
    return;
  }
  ...
  const files: FileInput[] = Array.isArray(req.body?.files) ? req.body.files : [];
  const spec = REQUIRED[eventType];

  // ── Validation ──
  const missing: string[] = [];
  if (spec.gps && (!gps || gps.lat == null || gps.long == null)) missing.push("gps");
  ...
  if (missing.length > 0) {
    ...
    res.status(422).json({ error: "Missing required items", eventType, missing });
    return;
  }
  ...
  const result = await db.transaction(async (tx) => {
    ...
    for (const f of files) {
      const [row] = await tx.insert(deliveryEvidenceTable).values({
        jobId,
    ...
    await recordJobTimelineEvent(jobId, profile.id, TIMELINE_STATUS[eventType], {
      ticketId,
