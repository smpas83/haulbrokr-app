...
import { db, deviceTokensTable, activityTable, profilesTable } from "@workspace/db";
...
router.put("/notifications/preferences", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  ...
  const updated = await upsertNotificationPreferences(profile.id, parsed.data);
 ...
 */
router.post("/notifications/broadcast", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  if (!profile.organizationId) {
    res.status(400).json({ error: "You must belong to an organization to broadcast." });
    return;
  }

  const Audience = z.object({
    roles: z.array(z.enum(["driver", "customer", "dispatcher", "fleet_manager", "provider", "supervisor"])).min(1),
    title: z.string().min(1).max(120),
    description: z.string().min(1).max(500),
    topic: z.enum(["job", "payment", "bid", "compliance", "reminder", "marketing", "general"]).optional(),
  });
  ...
  const sent = await notifyOrgRoles(profile.organizationId, parsed.data.roles, {
    type: "job_reminder",
...
router.post("/notifications/test", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  await notifyUser({
