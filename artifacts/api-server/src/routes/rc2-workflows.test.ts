...
describe("RC2 Phase 1 — workflow certification", () => {
  ...
  it("19. Recurring haul execution — worker surface present (RC3)", () => {
    const apiSrc = join(process.cwd(), "src/routes");
    const index = readFileSync(join(apiSrc, "index.ts"), "utf8");
    expect(index).toMatch(/recurringRouter|recurring/);
    expect(existsSync(join(process.cwd(), "src/lib/recurringHauls.ts"))).toBe(true);
    expect(existsSync(join(process.cwd(), "src/routes/workers.ts"))).toBe(true);
  });
});
