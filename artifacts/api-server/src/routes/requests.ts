...
router.get("/requests", requireProfile, async (req, res): Promise<void> => {
  ...
  if (params.success && params.data.mine) {
    rows = await db
      .select()
      .from(requestsTable)
      .where(eq(requestsTable.customerId, profile.id))
  ...
  const enriched = await Promise.all(
    rows.map(async (r) => {
      const [customer] = await db
        .select()
        .from(profilesTable)
        .where(eq(profilesTable.id, r.customerId));
...
router.post("/requests", requireProfile, async (req, res): Promise<void> => {
  ...
  const [request] = await db
    ...
    .values({
      ...
      budgetPerHour:
        parsed.data.budgetPerHour != null
          ? String(parsed.data.budgetPerHour)
          : undefined,
    })
    .returning();
...
router.get("/requests/:id", requireProfile, async (req, res): Promise<void> => {
  ...
  const bidCountResult = await db
    .select({ count: sql<number>`count(*)` })
    .from(bidsTable)
    .where(eq(bidsTable.requestId, request.id));
  res.json(
  ...
  async (req, res): Promise<void> => {
    ...
    const [request] = await db
      ...
      .set({
        ...parsed.data,
        quantityTons:
          parsed.data.quantityTons != null
            ? String(parsed.data.quantityTons)
            : undefined,
        estimatedHours:
