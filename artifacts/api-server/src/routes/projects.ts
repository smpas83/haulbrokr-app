...
router.get("/projects/:id", requireProfile, async (req, res): Promise<void> => {
  ...
  const [project] = await db
    ...
    .from(projectsTable)
    .where(
      and(eq(projectsTable.id, id), eq(projectsTable.customerId, profile.id)),
    );
  if (!project) {
  ...
  const jobs =
    requests.length > 0
      ? await db
          ...
          .from(jobsTable)
          .where(
            sql`${jobsTable.requestId} = ANY(${requests.map((r) => r.id)})`,
          )
      : [];
...
async function loadProjectIfOwned(projectId: number, profile: any) {
  ...
  if (profile.organizationId) {
    const [owner] = await db
      .select()
      .from(profilesTable)
      .where(eq(profilesTable.id, project.customerId));
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const rows = await db
      ...
      .leftJoin(
        profilesTable,
        eq(profilesTable.id, projectAssignmentsTable.supervisorProfileId),
      )
      .where(eq(projectAssignmentsTable.projectId, projectId));

    const enriched = rows.map(
      ({ supervisorCompany, supervisorName, ...r }) => ({
        ...r,
        supervisorName: supervisorName ?? supervisorCompany,
      }),
    );
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (!isOrgManager(profile)) {
      res
        .status(403)
        .json({ error: "Only the owner or an admin can assign foremen." });
      return;
    }
    const project = await loadProjectIfOwned(projectId, profile);
    if (!project) {
    ...
    const [foreman] = await db
      .select()
      .from(profilesTable)
      .where(eq(profilesTable.id, parsed.data.supervisorProfileId));
    if (
      !foreman ||
      foreman.role !== "supervisor" ||
      foreman.organizationId !== profile.organizationId
    ) {
      res
        .status(400)
        .json({ error: "Foreman must be a supervisor in your company." });
      return;
    }
    ...
    const [existing] = await db
      ...
      .where(
        and(
          ...
          eq(
            projectAssignmentsTable.supervisorProfileId,
    ...
    if (existing) {
      res.status(201).json({
        ...existing,
        supervisorName: foreman.contactName ?? foreman.companyName,
      });
      return;
    }
...
router.delete(
  "/projects/:id/assignments/:profileId",
  requireProfile,
  async (req, res): Promise<void> => {
    const profile = getRequestProfile(req);
    const projectId = parseInt(String(req.params.id), 10);
    ...
    if (!isOrgManager(profile)) {
      res
        .status(403)
        .json({ error: "Only the owner or an admin can unassign foremen." });
      return;
    }
    const project = await loadProjectIfOwned(projectId, profile);
    if (!project) {
