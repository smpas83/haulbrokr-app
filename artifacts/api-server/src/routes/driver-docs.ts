...
router.put("/driver-docs/:docType", requireProfile, async (req, res): Promise<void> => {
  ...
  if (objectPath !== undefined) {
    if (!storageToken) {
      res.status(400).json({ error: "storageToken is required when providing objectPath" });
      return;
    }

    const tokenResult = verifyStorageToken(storageToken, String(profile.id), objectPath);
    if (!tokenResult.ok) {
      res.status(403).json({ error: "Invalid or expired storage token", reason: tokenResult.error });
      return;
    }
    ...
    if (tokenGeneration) {
      ...
      try {
        ...
        if (err instanceof ObjectNotFoundError) {
          res.status(422).json({ error: "Document object not found in storage" });
        } else {
      ...
      if (currentGeneration !== tokenGeneration) {
        res.status(409).json({
          error: "Document object has been modified since it was verified. Please re-upload.",
        });
        return;
      }
    }
  ...
  const [existing] = await db.select().from(driverDocumentsTable)
    .where(and(eq(driverDocumentsTable.profileId, profile.id), eq(driverDocumentsTable.docType, docType)));
