...
import { retryFailedRecurringGenerations, runRecurringHaulWorker } from "../lib/recurringHauls";
 ...
 */
router.post("/workers/recurring-hauls", requireAutomationKey, async (req, res): Promise<void> => {
  try {
    const scheduleIds = Array.isArray(req.body?.scheduleIds)
      ? req.body.scheduleIds.map(Number).filter((n: number) => Number.isFinite(n))
      : undefined;
...
router.post("/workers/expire-exports", requireAutomationKey, async (_req, res): Promise<void> => {
  try {
...
router.get("/workers/fmcsa-status", requireAutomationKey, async (_req, res): Promise<void> => {
