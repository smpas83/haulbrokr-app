...
const CreateBody = z.object({
  materialType: z.enum(["dirt", "gravel", "sand", "concrete", "asphalt", "demolition", "topsoil", "fill", "other"]),
  truckType: TruckType,
...
router.get("/recurring-hauls", requireProfile, async (req, res): Promise<void> => {
...
router.get("/recurring-hauls/calendar", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
...
router.post("/recurring-hauls/run-scheduler", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
...
router.post("/recurring-hauls", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  if (profile.role !== "customer" && !canDispatch(profile)) {
    res.status(403).json({ error: "Only customers or dispatchers can create recurring hauls." });
    return;
  }
...
router.get("/recurring-hauls/:id", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  ...
  const [row] = await db.select().from(recurringHaulsTable).where(eq(recurringHaulsTable.id, id));
...
router.patch("/recurring-hauls/:id", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  const id = parseInt(String(req.params.id), 10);
  if (!Number.isFinite(id)) {
    res.status(400).json({ error: "Invalid id" });
  ...
  const [existing] = await db.select().from(recurringHaulsTable).where(eq(recurringHaulsTable.id, id));
  ...
  if (d.estimatedHours != null) patch.estimatedHours = String(d.estimatedHours);
  if (d.trucksNeeded != null) patch.trucksNeeded = d.trucksNeeded;
  if (d.budgetPerHour !== undefined) patch.budgetPerHour = d.budgetPerHour != null ? String(d.budgetPerHour) : null;
  if (d.notes !== undefined) patch.notes = d.notes;
  if (d.projectId !== undefined) patch.projectId = d.projectId;
  if (d.frequency != null) patch.frequency = d.frequency;
  if (d.daysOfWeek !== undefined) patch.daysOfWeek = d.daysOfWeek?.length ? d.daysOfWeek.join(",") : null;
  ...
  if (d.reminderHoursBefore !== undefined) patch.reminderHoursBefore = d.reminderHoursBefore;
  if (d.maxOccurrences !== undefined) patch.maxOccurrences = d.maxOccurrences;
  if (d.status != null) patch.status = d.status;

  if (d.frequency != null || d.daysOfWeek !== undefined || d.dayOfMonth !== undefined || d.startDate != null) {
    const merged = {
      frequency: (patch.frequency as typeof existing.frequency) ?? existing.frequency,
      daysOfWeek: (patch.daysOfWeek as string | null | undefined) ?? existing.daysOfWeek,
      dayOfMonth: (patch.dayOfMonth as number | null | undefined) ?? existing.dayOfMonth,
      startDate: (patch.startDate as Date) ?? existing.startDate,
      endDate: (patch.endDate as Date | null | undefined) !== undefined ? (patch.endDate as Date | null) : existing.endDate,
    };
  ...
  const [updated] = await db
    ...
    .set(patch)
    .where(and(eq(recurringHaulsTable.id, id), eq(recurringHaulsTable.customerId, profile.id)))
    .returning();
  res.json(serializeRecurringHaul(updated));
});

router.delete("/recurring-hauls/:id", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  const id = parseInt(String(req.params.id), 10);
  if (!Number.isFinite(id)) {
    res.status(400).json({ error: "Invalid id" });
