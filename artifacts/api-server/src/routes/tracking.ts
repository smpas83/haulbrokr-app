...
function parseGpsNote(note: string | null): { lat: number; lng: number; at: string } | null {
  if (!note) return null;
  ...
  return {
    lat: parseFloat(m[1]),
    lng: parseFloat(m[2]),
    at: m[3] ? new Date(parseInt(m[3], 10)).toISOString() : new Date().toISOString(),
  };
}

/** Driver pings live location during active haul. */
router.post("/jobs/:id/location", requireProfile, async (req, res): Promise<void> => {
  ...
  if (!DRIVER_SIDE.has(profile.role)) {
    res.status(403).json({ error: "Only drivers and providers can update location." });
    return;
  }
...
router.get("/jobs/:id/tracking", requireProfile, async (req, res): Promise<void> => {
  ...
  const trail = [
    ...updates.map((u) => ({ source: "status" as const, status: u.status, ...parseGpsNote(u.note), createdAt: u.createdAt })),
    ...evidence.map((e) => ({ source: "evidence" as const, ...parseGpsNote(e.siteNotes), createdAt: e.uploadedAt })),
...
router.get("/dispatch/overview", requireProfile, async (req, res): Promise<void> => {
  const profile = getRequestProfile(req);
  ...
  if (jobIds.length > 0) {
    ...
    for (const u of updates) {
      ...
      if (gps) positions[u.jobId] = { lat: gps.lat, lng: gps.lng, at: gps.at };
    }
  }
