...
function verifyToken(token: string): { ok: true; j: number; t: number; n: string; e: number } | { ok: false; error: string } {
  ...
  const expectedSig = crypto.createHmac("sha256", getSecret()).update(payloadJson).digest();
...
router.post("/jobs/:id/tickets", requireProfile, async (req, res): Promise<void> => {
  ...
  if (profile.role === "driver") {
    const [assigned] = await db
      ...
      .from(ticketsTable)
      .where(and(eq(ticketsTable.jobId, jobId), eq(ticketsTable.driverProfileId, profile.id)));
    if (!assigned) {
  ...
  const body = req.body ?? {};
  const weight = body.weightTons !== undefined && body.weightTons !== null ? String(body.weightTons) : null;
...
router.post("/tickets/:id/qr", requireProfile, async (req, res): Promise<void> => {
  ...
  const found = await loadOwnedTicket(id, profile);
  if (!found) { res.status(404).json({ error: "Ticket not found" }); return; }
  if (found.ticket.verifiedAt) { res.status(409).json({ error: "Ticket already verified — no new QR can be issued." }); return; }

  const nonce = crypto.randomBytes(12).toString("hex");
  const exp = Date.now() + TICKET_QR_TTL_MS;
  const token = signPayload({ j: found.ticket.jobId, t: found.ticket.id, n: nonce, e: exp });
  const [updated] = await db.update(ticketsTable)
    .set({ qrNonce: nonce, qrIssuedAt: new Date(), qrExpiresAt: new Date(exp) })
    .where(eq(ticketsTable.id, id))
    .returning();
...
router.post("/tickets/verify", requireProfile, async (req, res): Promise<void> => {
  ...
  if (!job) { res.status(403).json({ error: "You don't have access to this job." }); return; }
  if (!CUSTOMER_SIDE.has(profile.role) || (profile.role === "supervisor" && job.customerId !== profile.id && (await db.select().from(profilesTable).where(eq(profilesTable.id, job.customerId)))[0]?.organizationId !== profile.organizationId)) {
    res.status(403).json({ error: "You aren't on the customer side of this job." });
    return;
  }
  const [ticket] = await db.select().from(ticketsTable).where(eq(ticketsTable.id, v.t));
  if (!ticket || ticket.jobId !== v.j) { res.status(404).json({ error: "Ticket not found." }); return; }
  ...
  if (!updated) {
    if (ticket.verifiedAt) {
      res.status(409).json({ error: `Ticket #${ticket.loadNumber} was already verified at ${ticket.verifiedAt.toISOString()}.` });
    } else {
      res.status(409).json({ error: "QR code is stale — ask the driver to refresh it." });
    }
    return;
  }
  res.json({ ok: true, ticket: updated, verifierName: profile.contactName ?? profile.companyName });
});
