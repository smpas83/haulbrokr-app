 ...
 */
async function notifyPaymentRequiresAction(job: {
  id: number;
  customerId: number;
  materialType: string;
}): Promise<void> {
...
export function computeBreakdown(
  ...
  const amounts = computeJobAmounts(ratePerHour, hours, feeRate);
  return {
    base: amounts.vendorPayout,
    fee: amounts.platformCommission,
...
async function settleProviderPayout(
  job: any,
  ...
  const hasRealInstrument =
    !!customer.stripeCustomerId && !!customer.stripePaymentMethodId;
  ...
  const pi = await stripe.paymentIntents.create(chargeParams as any, {
  ...
  const transfer = await stripe.transfers.create(
    {
      ...
      description: `HaulBrokr payout for job #${job.id} (net of marketplace commission)`,
      metadata: { jobId: String(job.id), attempt: String(attempt) },
    },
  ...
  await recordPaymentTransaction({
    ...
    idempotencyKey: `job-charge:${job.id}:${attempt}`,
    attempt,
    metadata: { settlement: "direct_charge" },
  });
  await recordPaymentTransaction({
    jobId: job.id,
    kind: "transfer",
    ...
    idempotencyKey: `job-transfer:${job.id}:${attempt}`,
    attempt,
    metadata: { settlement: "connect_transfer" },
  });
  await recordPayoutTransfer({
    jobId: job.id,
    providerProfileId: job.providerId,
    stripeAccountId,
    stripeTransferId: transfer.id,
    amountCents: netCents,
...
router.get("/jobs/:id", requireProfile, async (req, res): Promise<void> => {
  ...
  res.json(
    GetJobResponse.parse(serializeJob(job, customerCompany, providerCompany)),
  );
});
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const docs = await db
      ...
      .where(
        and(
          ...
          inArray(
            driverDocumentsTable.docType,
            CUSTOMER_VISIBLE_DOC_TYPES as unknown as string[],
          ),
        ),
      );
    ...
    res.json({
      ...
      providerCompany:
        providerCompany?.companyName ?? providerCompany?.contactName ?? null,
      documents: docs,
    });
  },
);
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${built.invoiceNumber}.pdf"`,
      );
      res.send(Buffer.from(built.pdf));
    } catch (err) {
      req.log?.error?.(
        { err, jobId: job.id },
        "Job invoice PDF generation failed",
      );
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (profile.role === "provider" && job.providerId !== profile.id) {
      res
        .status(403)
        .json({ error: "Only the awarded hauler may accept this job" });
      return;
    }
    ...
    const [updated] = await db
      ...
      .set({ status: "accepted" })
      .where(
        and(eq(jobsTable.id, job.id), eq(jobsTable.providerId, profile.id)),
      )
      .returning();
    if (!updated) {
      res.status(404).json({ error: "Job not found" });
    ...
    res.json(
      AcceptJobResponse.parse(
        serializeJob(updated, customerCompany, providerCompany),
      ),
    );
  },
);
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (profile.role === "provider" && job.providerId !== profile.id) {
      res
        .status(403)
        .json({ error: "Only the awarded hauler may decline this job" });
      return;
    }
    if (job.status !== "awarded") {
      res.status(400).json({ error: "Job is not awaiting hauler acceptance" });
    ...
    const pendingBids = await db
      ...
      .where(
        and(
          eq(bidsTable.requestId, job.requestId),
...
router.patch("/jobs/:id", requireProfile, async (req, res): Promise<void> => {
  ...
  if (
    isAssignedDriver &&
    !isProvider &&
    (parsed.data.totalHours !== undefined || parsed.data.notes !== undefined)
  ) {
  ...
  if (
    ...
    !["accepted", "active"].includes(existingJob.status)
  ) {
  ...
  if (parsed.data.totalHours !== undefined)
    updates.totalHours = String(parsed.data.totalHours);
  ...
  if (parsed.data.status === "completed") {
    ...
    await recordJobTimelineEvent(job.id, profile.id, "completed", {
    ...
    await recordJobTimelineEvent(job.id, profile.id, "started", {
      note: "Work started",
    });
  }
  ...
  res.json(
    UpdateJobResponse.parse(
      serializeJob(job, customerCompany, providerCompany),
    ),
  );
});
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (job.status !== "completed") {
      res
        .status(400)
        .json({ error: "Job must be completed before it can be charged." });
      return;
    }
    ...
    const [pm] = await db
      ...
      .where(eq(paymentMethodsTable.profileId, job.customerId));
    const netDaysByMethod: Record<string, number> = {
      net_15: 15,
      net_30: 30,
      net_45: 45,
    };
    ...
    if (savedIsNetTerms) {
      const [creditApp] = await db
        ...
        .where(eq(creditApplicationsTable.profileId, job.customerId));
    ...
    try {
      const { paymentIntentId, chargeId, transferId } =
        await settleProviderPayout(
          job,
      ...
      if (authPi) {
        ...
        const { customerCompany, providerCompany } =
          await companiesFor(updated);
        res.json(
          ChargeJobResponse.parse(
            serializeJob(updated, customerCompany, providerCompany),
          ),
      ...
      await db
        .update(jobsTable)
        .set({ paymentStatus: "failed", paymentAttempts: attempt })
        .where(eq(jobsTable.id, job.id));
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const hasOpenInvoice =
      ...
      (job.paymentStatus === "failed" && job.invoicedAt != null);
    if (!hasOpenInvoice) {
      res
        .status(400)
        .json({ error: "There is no open invoice to release for this job." });
      return;
    }
    if (job.customerTotalAmount == null || job.providerNetAmount == null) {
    ...
    try {
      ...
      const { customerCompany, providerCompany } = await companiesFor(updated);
      res.json(
        ReleaseJobPaymentResponse.parse(
          serializeJob(updated, customerCompany, providerCompany),
        ),
      );
    } catch (err: any) {
 ...
 */
router.get(
  "/jobs/:id/payment-confirmation",
  requireProfile,
  async (req, res): Promise<void> => {
    const profile = getRequestProfile(req);
    const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const params = GetJobPaymentConfirmationParams.safeParse({
      id: parseInt(raw, 10),
    });
    if (!params.success) {
      res.status(400).json({ error: params.error.message });
    ...
    if (job.paymentStatus !== "requires_action" || !job.stripePaymentIntentId) {
      res
        .status(409)
        .json({ error: "This job has no payment awaiting confirmation." });
      return;
    }
    const stripe = await getUncachableStripeClient();
    ...
    res.json(
      GetJobPaymentConfirmationResponse.parse({
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      const { customerCompany, providerCompany } = await companiesFor(updated);
      res.json(
        ConfirmJobPaymentResponse.parse(
          serializeJob(updated, customerCompany, providerCompany),
        ),
      );
    } catch (err: any) {
      ...
      res.status(502).json({
        error:
          "Your payment was confirmed, but releasing the provider payout didn't complete. Please try again in a moment.",
      });
    }
  },
);
 ...
 */
router.post(
  "/jobs/:id/checkout-session",
  requireProfile,
  async (req, res): Promise<void> => {
    const profile = getRequestProfile(req);
    const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const params = CreateJobCheckoutSessionParams.safeParse({
      id: parseInt(raw, 10),
    });
    if (!params.success) {
      res.status(400).json({ error: params.error.message });
    ...
    const grossCents = Math.round(parseFloat(job.customerTotalAmount) * 100);
    const netCents = Math.round(parseFloat(job.providerNetAmount) * 100);
    const feeCents = grossCents - netCents; // retained marketplace commission
    ...
    try {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        line_items: [
          {
            price_data: {
              ...
              product_data: {
                ...
                description:
                  "Includes the configured HaulBrokr marketplace commission.",
              },
            },
            quantity: 1,
          },
        ...
        payment_intent_data: {
          ...
          description: `HaulBrokr job #${job.id} (gross, net of marketplace commission routed to provider)`,
      ...
      res
        .status(502)
        .json({ error: err?.message ?? "Could not start Checkout." });
    }
  },
);
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (job.paymentStatus === "released" || job.paymentStatus === "paid") {
      const { customerCompany, providerCompany } = await companiesFor(job);
      res.json(
        VerifyJobCheckoutResponse.parse(
          serializeJob(job, customerCompany, providerCompany),
        ),
      );
      return;
    ...
    try {
      ...
      const transferId =
        charge && typeof charge.transfer === "string"
          ? charge.transfer
      ...
      await recordPaymentTransaction({
        ...
        kind: "checkout",
        status: "succeeded",
        amountCents: Math.round(parseFloat(job.customerTotalAmount) * 100),
        stripeCheckoutSessionId: session.id,
        stripePaymentIntentId: paymentIntentId,
        stripeChargeId: chargeId,
      });
      await recordPaymentTransaction({
        ...
        stripeChargeId: chargeId,
        stripeTransferId: transferId,
        metadata: { settlement: "checkout_destination_charge" },
      });
      if (transferId) {
        await recordPaymentTransaction({
          jobId: job.id,
          kind: "transfer",
          status: "succeeded",
          amountCents: Math.round(parseFloat(job.providerNetAmount) * 100),
          stripeCheckoutSessionId: session.id,
          stripePaymentIntentId: paymentIntentId,
          stripeChargeId: chargeId,
          stripeTransferId: transferId,
      ...
      res
        .status(502)
        .json({ error: err?.message ?? "Could not verify the checkout." });
    }
  },
);
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const [driver] = await db
      .select()
      .from(profilesTable)
      .where(eq(profilesTable.id, parsed.data.driverProfileId));
    ...
    const driverOk =
      ...
      (driver.id === job.providerId ||
        (driver.role === "driver" &&
          ...
          driver.organizationId === providerProfile?.organizationId));
    if (!driverOk) {
      res.status(400).json({ error: "Driver is not part of your company." });
      return;
    }
    ...
    if (parsed.data.truckId != null) {
      ...
      await db
        .update(trucksTable)
        .set({ assignedDriverId: parsed.data.driverProfileId })
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const rows = await db
      ...
      .leftJoin(
        profilesTable,
        eq(profilesTable.id, jobStatusUpdatesTable.actorProfileId),
      )
      .where(eq(jobStatusUpdatesTable.jobId, jobId))
      .orderBy(sql`${jobStatusUpdatesTable.createdAt} asc`);

    const enriched = rows.map(({ actorCompany, actorName, ...r }) => ({
      ...r,
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (!CUSTOMER_SIDE.has(profile.role)) {
      res
        .status(403)
        .json({ error: "Only a customer or foreman can approve completion." });
      return;
    }
    const job = await loadJobIfMember(jobId, profile);
    if (!job) {
    ...
    const { customerCompany, providerCompany } = await companiesFor(updated);
    res.json(
      ApproveJobCompletionResponse.parse(
        serializeJob(updated, customerCompany, providerCompany),
      ),
    );
  },
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (!CUSTOMER_SIDE.has(profile.role)) {
      res
        .status(403)
        .json({ error: "Only a customer or foreman can flag completion." });
      return;
    }
    const job = await loadJobIfMember(jobId, profile);
    if (!job) {
    ...
    const { customerCompany, providerCompany } = await companiesFor(updated);
    res.json(
      FlagJobCompletionResponse.parse(
        serializeJob(updated, customerCompany, providerCompany),
      ),
    );
  },
