...
function reminderHtml(companyName: string, missing: string[]): string {
  const items = missing.map((m) => "<li style=\"margin:4px 0;\">" + m + "</li>").join("");
  return [
    ...
    '<p style="margin:20px 0;"><a href="' + DOC_SETTINGS_URL + '" style="background:#0f172a;color:#fff;padding:10px 18px;text-decoration:none;border-radius:4px;display:inline-block;">Upload your documents</a></p>',
 ...
 */
export async function sweepDocumentReminders(): Promise<{ checked: number; sent: number }> {
  ...
  const candidates = await db
    ...
    .where(
      and(
        or(eq(profilesTable.role, "provider"), eq(profilesTable.role, "customer")),
        sql`${profilesTable.email} is not null and ${profilesTable.email} <> ''`,
        or(isNull(profilesTable.lastDocReminderAt), lt(profilesTable.lastDocReminderAt, cutoff)),
      ),
    );
  ...
  let client: Awaited<ReturnType<typeof getUncachableResendClient>> | null = null;
  for (const profile of candidates) {
    try {
      ...
      logger.error({ err, profileId: profile.id }, "Document reminder email failed");
    }
  }
  logger.info({ checked: candidates.length, sent }, "Document reminder sweep complete");
