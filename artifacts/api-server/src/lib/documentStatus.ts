 ...
 */
export async function computeDocumentStatus(
  profile: Profile,
    ...
    const items: RequiredDocItem[] = [
      {
        key: "w9",
        label: "W-9 tax form",
        status: snap?.w9Status ?? "not_submitted",
        satisfied: (snap?.w9Status ?? "") === "verified",
      },
      {
        key: "coi",
        label: "Certificate of Insurance (COI)",
        status: snap?.insuranceStatus ?? "not_submitted",
        satisfied: (snap?.insuranceStatus ?? "") === "verified",
      },
      {
        key: "dot_cdl",
        label: "DOT / CDL authority",
        status: snap?.dotCdlStatus ?? "not_submitted",
        satisfied: (snap?.dotCdlStatus ?? "") === "verified",
      },
      {
        key: "payout",
        label: "Verified payout account",
        status: snap?.payoutStatus ?? "not_submitted",
        satisfied: (snap?.payoutStatus ?? "") === "verified",
      },
    ];
...
export async function computeDocumentStatusById(
  profileId: number,
