...
export const PRODUCTION_ENV_REQUIREMENTS: EnvRequirement[] = [
  ...
  { service: "core", variable: "GOOGLE_MAPS_API_KEY", required: true, description: "Google Maps Platform key for server geocoding, Routes API, and Distance Matrix (no Haversine fallback in production)." },
...
function validateCoreSecrets(env: NodeJS.ProcessEnv, issues: EnvValidationIssue[]): void {
  ...
  if (!mapsKey) {
    ...
    pushInvalid(issues, "core", "GOOGLE_MAPS_API_KEY", "GOOGLE_MAPS_API_KEY looks too short for a Google Maps Platform key.");
  }
}
