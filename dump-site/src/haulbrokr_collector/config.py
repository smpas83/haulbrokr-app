...
class ExportConfig(BaseModel):
    ...
    supabase_table: str = "dump_sites"
    supabase_upsert: bool = True


class SupabaseConfig(BaseModel):
    """Optional live Supabase push (also reads SUPABASE_URL / SUPABASE_KEY env)."""

    url: str = ""
    key: str = ""
    table: str = "dump_sites"
    enabled: bool = False
...
def load_config(path: Path | str | None = None) -> CollectorConfig:
    ...
    # Environment overrides for Supabase credentials.
    ...
    if env_key:
        config.supabase.key = env_key
    if config.supabase.url and config.supabase.key:
        config.supabase.enabled = True
    if not config.supabase.table:
        config.supabase.table = config.export.supabase_table
    return config
