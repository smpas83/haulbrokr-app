...
| Email | **Resend** | — |
| File uploads | **Cloudflare R2** | private + public prefixes |
...
Create a [Cloudflare R2](https://www.cloudflare.com/products/r2/) bucket before launch. Production uploads require R2 credentials, a private prefix, and a public prefix.
...
7. Wait for deploy → verify:
   ```bash
   curl https://haulbrokr-api.onrender.com/api/healthz
   # → {"status":"ok"}
   ```

8. If your Render service name differs from `haulbrokr-api`, update the rewrite URL in `artifacts/haulbrokr/vercel.json`.
9. Run `DATABASE_URL="postgres://..." pnpm --filter @workspace/db run push` before the API deploy whenever schema/index definitions change.
...
```bash
curl https://haulbrokr.com/api/healthz
...
- [ ] `curl https://haulbrokr.com/api/healthz` returns ok
...
| Problem | Fix |
|---------|-----|
| `/api/healthz` 502 on Vercel | Render API not running — check Render logs |
