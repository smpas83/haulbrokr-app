...
export const insuranceSubmissionsTable = pgTable("insurance_submissions", {
  ...
  glCoverageAmount: numeric("gl_coverage_amount", {
    precision: 14,
    scale: 2,
  }).notNull(),
  glExpirationDate: timestamp("gl_expiration_date", {
    withTimezone: true,
  }).notNull(),
  autoCarrier: text("auto_carrier"),
  autoPolicyNumber: text("auto_policy_number"),
  autoCoverageAmount: numeric("auto_coverage_amount", {
    precision: 14,
    scale: 2,
  }),
  ...
  status: verificationStatusEnum("insurance_status")
    .notNull()
    .default("pending"),
...
export const insertInsuranceSchema = createInsertSchema(
