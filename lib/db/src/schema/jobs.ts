...
export const jobCompletionApprovalEnum = pgEnum("job_completion_approval", [
  "pending",
  "approved",
  "flagged",
]);
...
export const jobsTable = pgTable("jobs", {
  ...
  platformFeeRate: numeric("platform_fee_rate", { precision: 5, scale: 4 })
    .notNull()
    .default("0.15"),
  ...
  customerTotalAmount: numeric("customer_total_amount", {
    precision: 12,
    scale: 2,
  }),
  providerNetAmount: numeric("provider_net_amount", {
    precision: 12,
    scale: 2,
  }),
  ...
  // Cumulative amount refunded to the customer across all refund legs.
  refundedAmount: numeric("refunded_amount", { precision: 12, scale: 2 })
    .notNull()
    .default("0"),
  // Incremented on each refund attempt so Stripe idempotency keys stay unique.
  ...
  approvedByProfileId: integer("approved_by_profile_id").references(
    () => profilesTable.id,
  ),
